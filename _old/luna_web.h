...
static void luna_web_frame(void*ud){(void)ud;if(!luna_web.running)return;double now=luna_web_time_impl(),dt=now-luna_web.previous;luna_web.previous=now;if(dt<0||dt>.25)dt=1.0/60.0;if(luna_web.config.on_frame)luna_web.config.on_frame(dt,luna_web.config.userdata);luna_update(now,dt);int w=0,h=0;emscripten_get_canvas_element_size(LUNA_WEB_CANVAS,&w,&h);if(w>0&&h>0)luna_render(w,h);}
