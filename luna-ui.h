...
/*
 * The public entry point auto-selects a header-only native host. Define
 * LUNA_UI_NO_PLATFORM before including this file when embedding the renderer
 * in an existing window/event system.
 */
#if !defined(LUNA_UI_NO_PLATFORM) && defined(LUNA_UI_IMPLEMENTATION)
#  define LUNA_UI_PLATFORM_PRELUDE 1
#  if defined(__EMSCRIPTEN__)
#    include "luna_web.h"
#  elif defined(__ANDROID__)
#    include "luna_android.h"
#  elif defined(__APPLE__)
#    include <TargetConditionals.h>
#    if TARGET_OS_IPHONE
#      include "luna_ios.h"
#    else
#      include "luna_macos.h"
#    endif
#  elif defined(_WIN32)
...
#  endif
#  undef LUNA_UI_PLATFORM_PRELUDE
#endif

#include <stddef.h>
#include <stdint.h>

/* A custom host may include its GL headers before this file. Native Luna
 * platform preludes do that automatically. */
#if !defined(LUNA_UI_PLATFORM_GL_INCLUDED) && !defined(GL_VERSION_1_1) && \
...
/* Constants missing from the legacy Windows OpenGL 1.1 header. */
...
typedef void (*LunaRequestRedrawFn)(void);
typedef unsigned char* (*LunaReadResourceFn)(const char* path, size_t* out_size);
typedef unsigned char* (*LunaLoadFontFn)(int role, size_t* out_size);
typedef int (*LunaSavePngFn)(const char* path, int width, int height,
                             int channels, const unsigned char* pixels,
                             int stride_bytes);
typedef void (*LunaSetClipboardFn)(const char* utf8);
...
typedef struct LunaPlatform {
    ...
    LunaRequestRedrawFn    request_redraw;
    LunaReadResourceFn     read_resource;
    LunaLoadFontFn         load_font;
    LunaSavePngFn          save_png;
    LunaSetClipboardFn     set_clipboard;
...
typedef struct LunaInitConfig {
    float width, height;
    LunaGetProcFn get_proc;
    /* Native hosts use this to select a borderless client area. Custom hosts
     * may ignore it because window creation occurs outside the core. */
    int frameless;
} LunaInitConfig;

/* Implemented by the automatically selected luna_*.h native host. */
...
/* Host-neutral input constants. Numeric values deliberately match GLFW 3.4. */
enum { LUNA_RELEASE = 0, LUNA_PRESS = 1, LUNA_REPEAT = 2 };
#define LUNA_DONT_CARE (-1)
enum {
    LUNA_MOUSE_BUTTON_LEFT = 0, LUNA_MOUSE_BUTTON_RIGHT = 1,
    LUNA_MOUSE_BUTTON_MIDDLE = 2, LUNA_MOUSE_BUTTON_4 = 3,
    LUNA_MOUSE_BUTTON_5 = 4, LUNA_MOUSE_BUTTON_6 = 5,
    LUNA_MOUSE_BUTTON_7 = 6, LUNA_MOUSE_BUTTON_8 = 7,
    LUNA_MOUSE_BUTTON_LAST = 7
};
enum {
    LUNA_MOD_SHIFT = 0x0001, LUNA_MOD_CONTROL = 0x0002,
    LUNA_MOD_ALT = 0x0004, LUNA_MOD_SUPER = 0x0008,
    LUNA_MOD_CAPS_LOCK = 0x0010, LUNA_MOD_NUM_LOCK = 0x0020
};
/* Physical key identity. Text arrives separately through luna_char(). */
enum {
    LUNA_KEY_UNKNOWN = -1,
    LUNA_KEY_SPACE = 32, LUNA_KEY_APOSTROPHE = 39, LUNA_KEY_COMMA = 44,
    LUNA_KEY_MINUS = 45, LUNA_KEY_PERIOD = 46, LUNA_KEY_SLASH = 47,
    LUNA_KEY_0 = 48, LUNA_KEY_1 = 49, LUNA_KEY_2 = 50, LUNA_KEY_3 = 51,
    LUNA_KEY_4 = 52, LUNA_KEY_5 = 53, LUNA_KEY_6 = 54, LUNA_KEY_7 = 55,
    LUNA_KEY_8 = 56, LUNA_KEY_9 = 57, LUNA_KEY_SEMICOLON = 59,
    LUNA_KEY_EQUAL = 61, LUNA_KEY_A = 65, LUNA_KEY_B = 66,
    LUNA_KEY_C = 67, LUNA_KEY_D = 68, LUNA_KEY_E = 69, LUNA_KEY_F = 70,
    LUNA_KEY_G = 71, LUNA_KEY_H = 72, LUNA_KEY_I = 73, LUNA_KEY_J = 74,
    LUNA_KEY_K = 75, LUNA_KEY_L = 76, LUNA_KEY_M = 77, LUNA_KEY_N = 78,
    LUNA_KEY_O = 79, LUNA_KEY_P = 80, LUNA_KEY_Q = 81, LUNA_KEY_R = 82,
    LUNA_KEY_S = 83, LUNA_KEY_T = 84, LUNA_KEY_U = 85, LUNA_KEY_V = 86,
    LUNA_KEY_W = 87, LUNA_KEY_X = 88, LUNA_KEY_Y = 89, LUNA_KEY_Z = 90,
    LUNA_KEY_LEFT_BRACKET = 91, LUNA_KEY_BACKSLASH = 92,
    LUNA_KEY_RIGHT_BRACKET = 93, LUNA_KEY_GRAVE_ACCENT = 96,
    LUNA_KEY_WORLD_1 = 161, LUNA_KEY_WORLD_2 = 162,
    LUNA_KEY_ESCAPE = 256, LUNA_KEY_ENTER = 257, LUNA_KEY_TAB = 258,
    LUNA_KEY_BACKSPACE = 259, LUNA_KEY_INSERT = 260, LUNA_KEY_DELETE = 261,
    LUNA_KEY_RIGHT = 262, LUNA_KEY_LEFT = 263, LUNA_KEY_DOWN = 264,
    LUNA_KEY_UP = 265, LUNA_KEY_PAGE_UP = 266, LUNA_KEY_PAGE_DOWN = 267,
    LUNA_KEY_HOME = 268, LUNA_KEY_END = 269, LUNA_KEY_CAPS_LOCK = 280,
    LUNA_KEY_SCROLL_LOCK = 281, LUNA_KEY_NUM_LOCK = 282,
    LUNA_KEY_PRINT_SCREEN = 283, LUNA_KEY_PAUSE = 284,
    LUNA_KEY_F1 = 290, LUNA_KEY_F2 = 291, LUNA_KEY_F3 = 292,
    LUNA_KEY_F4 = 293, LUNA_KEY_F5 = 294, LUNA_KEY_F6 = 295,
    LUNA_KEY_F7 = 296, LUNA_KEY_F8 = 297, LUNA_KEY_F9 = 298,
    LUNA_KEY_F10 = 299, LUNA_KEY_F11 = 300, LUNA_KEY_F12 = 301,
    LUNA_KEY_F13 = 302, LUNA_KEY_F14 = 303, LUNA_KEY_F15 = 304,
    LUNA_KEY_F16 = 305, LUNA_KEY_F17 = 306, LUNA_KEY_F18 = 307,
    LUNA_KEY_F19 = 308, LUNA_KEY_F20 = 309, LUNA_KEY_F21 = 310,
    LUNA_KEY_F22 = 311, LUNA_KEY_F23 = 312, LUNA_KEY_F24 = 313,
    LUNA_KEY_F25 = 314,
    LUNA_KEY_KP_0 = 320, LUNA_KEY_KP_1 = 321, LUNA_KEY_KP_2 = 322,
    LUNA_KEY_KP_3 = 323, LUNA_KEY_KP_4 = 324, LUNA_KEY_KP_5 = 325,
    LUNA_KEY_KP_6 = 326, LUNA_KEY_KP_7 = 327, LUNA_KEY_KP_8 = 328,
    LUNA_KEY_KP_9 = 329, LUNA_KEY_KP_DECIMAL = 330,
    LUNA_KEY_KP_DIVIDE = 331, LUNA_KEY_KP_MULTIPLY = 332,
    LUNA_KEY_KP_SUBTRACT = 333, LUNA_KEY_KP_ADD = 334,
    LUNA_KEY_KP_ENTER = 335, LUNA_KEY_KP_EQUAL = 336,
    LUNA_KEY_LEFT_SHIFT = 340, LUNA_KEY_LEFT_CONTROL = 341,
    LUNA_KEY_LEFT_ALT = 342, LUNA_KEY_LEFT_SUPER = 343,
    LUNA_KEY_RIGHT_SHIFT = 344, LUNA_KEY_RIGHT_CONTROL = 345,
    LUNA_KEY_RIGHT_ALT = 346, LUNA_KEY_RIGHT_SUPER = 347,
    LUNA_KEY_MENU = 348, LUNA_KEY_LAST = 348
};

/* Linux evdev scancode helpers used by the KMS host. */
static const short luna_evdev_to_key[256] = {
    -1, LUNA_KEY_ESCAPE, LUNA_KEY_1, LUNA_KEY_2, LUNA_KEY_3,
    LUNA_KEY_4, LUNA_KEY_5, LUNA_KEY_6, LUNA_KEY_7, LUNA_KEY_8,
    LUNA_KEY_9, LUNA_KEY_0, LUNA_KEY_MINUS, LUNA_KEY_EQUAL,
    LUNA_KEY_BACKSPACE, LUNA_KEY_TAB, LUNA_KEY_Q, LUNA_KEY_W,
    LUNA_KEY_E, LUNA_KEY_R, LUNA_KEY_T, LUNA_KEY_Y, LUNA_KEY_U,
    LUNA_KEY_I, LUNA_KEY_O, LUNA_KEY_P, LUNA_KEY_LEFT_BRACKET,
    LUNA_KEY_RIGHT_BRACKET, LUNA_KEY_ENTER, LUNA_KEY_LEFT_CONTROL,
    LUNA_KEY_A, LUNA_KEY_S, LUNA_KEY_D, LUNA_KEY_F, LUNA_KEY_G,
    LUNA_KEY_H, LUNA_KEY_J, LUNA_KEY_K, LUNA_KEY_L,
    LUNA_KEY_SEMICOLON, LUNA_KEY_APOSTROPHE, LUNA_KEY_GRAVE_ACCENT,
    LUNA_KEY_LEFT_SHIFT, LUNA_KEY_BACKSLASH, LUNA_KEY_Z, LUNA_KEY_X,
    LUNA_KEY_C, LUNA_KEY_V, LUNA_KEY_B, LUNA_KEY_N, LUNA_KEY_M,
    LUNA_KEY_COMMA, LUNA_KEY_PERIOD, LUNA_KEY_SLASH,
    LUNA_KEY_RIGHT_SHIFT, LUNA_KEY_KP_MULTIPLY, LUNA_KEY_LEFT_ALT,
    LUNA_KEY_SPACE, LUNA_KEY_CAPS_LOCK, LUNA_KEY_F1, LUNA_KEY_F2,
    LUNA_KEY_F3, LUNA_KEY_F4, LUNA_KEY_F5, LUNA_KEY_F6, LUNA_KEY_F7,
    LUNA_KEY_F8, LUNA_KEY_F9, LUNA_KEY_F10, LUNA_KEY_NUM_LOCK,
    LUNA_KEY_SCROLL_LOCK, LUNA_KEY_KP_7, LUNA_KEY_KP_8, LUNA_KEY_KP_9,
    LUNA_KEY_KP_SUBTRACT, LUNA_KEY_KP_4, LUNA_KEY_KP_5, LUNA_KEY_KP_6,
    LUNA_KEY_KP_ADD, LUNA_KEY_KP_1, LUNA_KEY_KP_2, LUNA_KEY_KP_3,
    LUNA_KEY_KP_0, LUNA_KEY_KP_DECIMAL, -1, -1, LUNA_KEY_WORLD_1,
    LUNA_KEY_F11, LUNA_KEY_F12, -1, -1, -1, -1, -1, -1, -1,
    LUNA_KEY_KP_ENTER, LUNA_KEY_RIGHT_CONTROL, LUNA_KEY_KP_DIVIDE,
    LUNA_KEY_PRINT_SCREEN, LUNA_KEY_RIGHT_ALT, -1, LUNA_KEY_HOME,
    LUNA_KEY_UP, LUNA_KEY_PAGE_UP, LUNA_KEY_LEFT, LUNA_KEY_RIGHT,
    LUNA_KEY_END, LUNA_KEY_DOWN, LUNA_KEY_PAGE_DOWN, LUNA_KEY_INSERT,
    LUNA_KEY_DELETE, -1, -1, -1, -1, -1, -1, -1,
    LUNA_KEY_PAUSE, -1, -1, -1, -1, LUNA_KEY_WORLD_2,
    LUNA_KEY_LEFT_SUPER, LUNA_KEY_RIGHT_SUPER, LUNA_KEY_MENU
};
static inline int luna_key_from_evdev(unsigned int code) {
    if (code >= 256 || luna_evdev_to_key[code] == 0) return LUNA_KEY_UNKNOWN;
...
/* Text metrics for a host that lays out its own widgets in luna-ui's type:
 * the advance of `text` (one line, UTF-8) at `css_px`, and CSS `normal` line
 * height at that size.  Font metrics only — no atlas is baked and no GL call
 * is made — but they read the loaded fonts, so a host calling from another
 * thread must serialise with its render. */
float luna_measure_text(const char* text, float css_px, int bold);
float luna_line_height(float css_px);
...
/* Topmost visible element at a viewport position, or -1. */
int  luna_element_at_point(double x, double y);
 ...
 * whole document layout, so a periodic label update cannot cause a frame hitch.
 * The host is responsible for requesting repaint of the owning surface. */
void luna_set_text_paint_only(int idx, const char* text);
...
/* 1 while a draggable element or scrollbar thumb owns the pointer.  Hosts can
...
/*
 * DOM mutation — lets a script engine use the document as its DOM.
 *
 * Indices are renumbered by luna_dom_insert() (the element array is kept in
 * document order, which is also paint order), so a caller that keeps a
 * reference keeps the element's uid and asks luna_dom_find() for the current
 * index.  A removed or newly created element is detached: it keeps its
 * subtree and its uid, is never laid out, painted or hit, and can be inserted
 * again.  Every call returns -1 / 0 on a bad argument and changes nothing.
 */
unsigned luna_dom_uid(int idx);
int  luna_dom_find(unsigned uid);
/* Web-document mode: HTML's default inline boxes (span, a, b, img, input…)
 * and text runs between elements as anonymous "#text" nodes.  Set before
 * parsing; Luna's own UIs leave it off. */
void luna_set_web_compat(int enabled);
/* A detached anonymous text node ("#text"); returns its index. */
int  luna_dom_create_text(const char* text);
/* A detached element of the given tag (with optional attributes in HTML
 * syntax, e.g. ` id="a" class="b"`); returns its index. */
int  luna_dom_create(const char* tag, const char* attrs);
/* Moves child (with its subtree) under parent, before `before` (a child of
 * parent) or last when before is -1.  parent -1 makes it a document root. */
int  luna_dom_insert(int parent, int child, int before);
int  luna_dom_remove(int idx);
int  luna_dom_is_connected(int idx);
/* Children in document order, engine-internal nodes (pseudo-elements,
 * scrollbars) excluded.  Returns the count; fills at most cap. */
int  luna_dom_children(int idx, int* out, int cap);
/* Replaces idx's children with the parsed fragment; returns the number of
 * top-level elements created. */
int  luna_dom_set_inner_html(int idx, const char* html);
/* Attribute access.  get returns 1 when present and copies the value. */
int  luna_dom_get_attr(int idx, const char* name, char* out, int cap);
int  luna_dom_set_attr(int idx, const char* name, const char* value);
int  luna_dom_remove_attr(int idx, const char* name);
/* The raw attribute list (` name="value"` ...) or "". */
const char* luna_dom_attrs(int idx);
/* Re-resolves the element's (and its subtree's) style after a change made
 * behind the engine's back. */
void luna_dom_restyle(int idx);
...
#ifndef LUNA_UI_ENABLE_FONT_SCAN
#  if defined(__linux__) || defined(__FreeBSD__)
#    define LUNA_UI_ENABLE_FONT_SCAN 1
#  else
#    define LUNA_UI_ENABLE_FONT_SCAN 0
#  endif
#endif

#if LUNA_UI_ENABLE_FONT_SCAN
#include <dirent.h>
#include <sys/stat.h>
#include <unistd.h>
#endif
...
#  define LUNA_UNUSED
#else
#  define LUNA_UNUSED __attribute__((unused))
#endif

#if defined(_WIN32)
#  define luna_strcasecmp  _stricmp
#  define luna_strncasecmp _strnicmp
#else
#  define luna_strcasecmp  strcasecmp
#  define luna_strncasecmp strncasecmp
#endif

static char* luna_strdup_local(const char* s) {
    size_t n;
    char* p;
    if (!s) return NULL;
    n = strlen(s) + 1;
...
static const char* luna_strcasestr_local(const char* haystack, const char* needle) {
    size_t n;
    if (!haystack || !needle) return NULL;
    if (!*needle) return haystack;
    n = strlen(needle);
    for (; *haystack; ++haystack)
        if (luna_strncasecmp(haystack, needle, n) == 0) return haystack;
    return NULL;
}
...
typedef void (*LunaPFNGLBUFFERDATAPROC)(GLenum target, GLsizeiptr size, const void* data, GLenum usage);
typedef void (*LunaPFNGLBUFFERSUBDATAPROC)(GLenum target, GLintptr offset, GLsizeiptr size, const void* data);
typedef void (*LunaPFNGLVERTEXATTRIBPOINTERPROC)(GLuint index, GLint size, GLenum type, GLboolean normalized, GLsizei stride, const void* pointer);
typedef void (*LunaPFNGLENABLEVERTEXATTRIBARRAYPROC)(GLuint index);
typedef void (*LunaPFNGLDELETEBUFFERSPROC)(GLsizei n, const GLuint* buffers);
typedef void (*LunaPFNGLBINDVERTEXARRAYPROC)(GLuint array);
typedef GLuint (*LunaPFNGLCREATESHADERPROC)(GLenum type);
typedef void (*LunaPFNGLSHADERSOURCEPROC)(GLuint shader, GLsizei count, const GLchar* const* string, const GLint* length);
typedef void (*LunaPFNGLCOMPILESHADERPROC)(GLuint shader);
typedef void (*LunaPFNGLGETSHADERIVPROC)(GLuint shader, GLenum pname, GLint* params);
typedef void (*LunaPFNGLGETSHADERINFOLOGPROC)(GLuint shader, GLsizei max_length, GLsizei* length, GLchar* info_log);
typedef void (*LunaPFNGLDELETESHADERPROC)(GLuint shader);
typedef GLuint (*LunaPFNGLCREATEPROGRAMPROC)(void);
typedef void (*LunaPFNGLATTACHSHADERPROC)(GLuint program, GLuint shader);
typedef void (*LunaPFNGLLINKPROGRAMPROC)(GLuint program);
typedef void (*LunaPFNGLGETPROGRAMIVPROC)(GLuint program, GLenum pname, GLint* params);
typedef void (*LunaPFNGLGETPROGRAMINFOLOGPROC)(GLuint program, GLsizei max_length, GLsizei* length, GLchar* info_log);
typedef void (*LunaPFNGLUSEPROGRAMPROC)(GLuint program);
typedef void (*LunaPFNGLDELETEPROGRAMPROC)(GLuint program);
typedef GLint (*LunaPFNGLGETUNIFORMLOCATIONPROC)(GLuint program, const GLchar* name);
typedef void (*LunaPFNGLUNIFORM4FPROC)(GLint location, GLfloat v0, GLfloat v1, GLfloat v2, GLfloat v3);
typedef void (*LunaPFNGLUNIFORM2FPROC)(GLint location, GLfloat v0, GLfloat v1);
typedef void (*LunaPFNGLUNIFORM1FPROC)(GLint location, GLfloat v0);
typedef void (*LunaPFNGLUNIFORM1IPROC)(GLint location, GLint v0);
typedef void (*LunaPFNGLACTIVETEXTUREPROC)(GLenum texture);
typedef void (*LunaPFNGLGENFRAMEBUFFERSPROC)(GLsizei n, GLuint* framebuffers);
typedef void (*LunaPFNGLBINDFRAMEBUFFERPROC)(GLenum target, GLuint framebuffer);
typedef void (*LunaPFNGLFRAMEBUFFERTEXTURE2DPROC)(GLenum target, GLenum attachment, GLenum textarget, GLuint texture, GLint level);
typedef void (*LunaPFNGLDELETEFRAMEBUFFERSPROC)(GLsizei n, const GLuint* framebuffers);
typedef GLenum (*LunaPFNGLCHECKFRAMEBUFFERSTATUSPROC)(GLenum target);
...
LunaPFNGLBUFFERDATAPROC luna_glBufferData;
LunaPFNGLBUFFERSUBDATAPROC luna_glBufferSubData;
LunaPFNGLVERTEXATTRIBPOINTERPROC luna_glVertexAttribPointer;
LunaPFNGLENABLEVERTEXATTRIBARRAYPROC luna_glEnableVertexAttribArray;
LunaPFNGLDELETEBUFFERSPROC luna_glDeleteBuffers;
LunaPFNGLBINDVERTEXARRAYPROC luna_glBindVertexArray;
LunaPFNGLCREATESHADERPROC luna_glCreateShader;
LunaPFNGLSHADERSOURCEPROC luna_glShaderSource;
LunaPFNGLCOMPILESHADERPROC luna_glCompileShader;
LunaPFNGLGETSHADERIVPROC glGetShaderiv_;
LunaPFNGLGETSHADERINFOLOGPROC glGetShaderInfoLog_;
LunaPFNGLDELETESHADERPROC glDeleteShader_;
LunaPFNGLCREATEPROGRAMPROC luna_glCreateProgram;
LunaPFNGLATTACHSHADERPROC luna_glAttachShader;
LunaPFNGLLINKPROGRAMPROC luna_glLinkProgram;
LunaPFNGLGETPROGRAMIVPROC glGetProgramiv_;
LunaPFNGLGETPROGRAMINFOLOGPROC glGetProgramInfoLog_;
LunaPFNGLUSEPROGRAMPROC luna_glUseProgram;
LunaPFNGLDELETEPROGRAMPROC glDeleteProgram_;
LunaPFNGLGETUNIFORMLOCATIONPROC luna_glGetUniformLocation;
...
#define glBufferData luna_glBufferData
#define glBufferSubData luna_glBufferSubData
#define glVertexAttribPointer luna_glVertexAttribPointer
#define glEnableVertexAttribArray luna_glEnableVertexAttribArray
#define glDeleteBuffers luna_glDeleteBuffers
#define glBindVertexArray luna_glBindVertexArray
#define glCreateShader luna_glCreateShader
#define glShaderSource luna_glShaderSource
#define glCompileShader luna_glCompileShader
#define glCreateProgram luna_glCreateProgram
#define glAttachShader luna_glAttachShader
#define glLinkProgram luna_glLinkProgram
#define glUseProgram luna_glUseProgram
#define glGetUniformLocation luna_glGetUniformLocation
#define glUniform4f luna_glUniform4f
#define glUniform2f luna_glUniform2f
#define glUniform1f luna_glUniform1f
LunaPFNGLUNIFORM1IPROC glUniform1i_;
LunaPFNGLACTIVETEXTUREPROC glActiveTexture_;
LunaPFNGLGENFRAMEBUFFERSPROC glGenFramebuffers_;
LunaPFNGLBINDFRAMEBUFFERPROC glBindFramebuffer_;
LunaPFNGLFRAMEBUFFERTEXTURE2DPROC glFramebufferTexture2D_;
LunaPFNGLDELETEFRAMEBUFFERSPROC glDeleteFramebuffers_;
LunaPFNGLCHECKFRAMEBUFFERSTATUSPROC glCheckFramebufferStatus_;
...
const char* bg_vs =
    ...
    "    vec2 localPos = vec2(aPos.x * uSize.x, aPos.y * uSize.y);\n"
    "    vec2 center = uSize * 0.5;\n"
    "    float c = cos(uRotate), s = sin(uRotate);\n"
    "    vec2 d = localPos - center;\n"
    "    vec2 screenPos = uPos + center + vec2(c*d.x - s*d.y, s*d.x + c*d.y);\n"
...
const char* img_fs =
    ...
    /* uRadius < 0: a hard-edged quad (a border-image piece meets its
     * neighbour exactly; an antialiased edge there shows as a seam). */
    "    float alpha = uRadius < 0.0 ? 1.0 : 1.0 - smoothstep(r - 1.0, r + 0.5, dist);\n"
    "    if(alpha <= 0.0) discard;\n"
...
/* display_mode is the *inner* display type; inline_level (below) is the outer
 * one.  `display:inline` / `inline-block` / `inline-flex` are an inline-level
 * box whose contents lay out as block / flex / grid. */
...
struct LunaElement {
    ...
    int letter_spacing_em; /* unresolved author value uses em until font-size is final */
    int line_clamp;       /* -webkit-line-clamp; 0 = none */
    ...
    int inline_level; // outer display: 1 inline, 2 inline-block/-flex/-grid
    int flex_direction;
    int justify_content;
    int align_items;
    int justify_items;
    ...
    /* CSS border-image (stretch): the image cut by `slice` into nine and
     * drawn into the border box with edges `width` wide. */
    int has_border_image;
    char border_image_path[256];
    GLuint border_image_tex;
    float bi_slice[4];              /* top right bottom left; image px or % */
    unsigned char bi_slice_pct;     /* bit i: bi_slice[i] is a percentage */
    float bi_width[4];              /* <0: the slice's own size */
    int bi_fill;
    ...
    /* DOM mutation API (luna_dom_*).  dom_uid is a handle that survives the
     * index renumbering a tree move performs (0 = engine-internal node);
     * dom_attrs keeps the element's source attributes (` name="value"` ...,
     * malloc'd) for getAttribute(); dom_detached marks the root of a subtree
     * that was removed from, or never inserted into, the document. */
    unsigned dom_uid;
    int dom_detached;
    char* dom_attrs;
    /* Inline formatting (web documents): anon_text marks an anonymous text
     * run materialized between element siblings; ws_before records source
     * whitespace before the box, which an inline line renders as a space. */
    int anon_text;
    int ws_before;
...
typedef struct {
    ...
    /* Dynamic pseudo-classes belong to the compound on which they appear.
       Keeping them here is required for selectors such as
       `.tooltip:hover .tooltip-text`; storing state only on the final target
       incorrectly tests the child instead of the hovered ancestor. */
    int  is_hover, is_active, is_focus, is_focus_visible, is_focus_within;
    int  is_root;
    int  unsupported_pseudo;         /* fail closed; never broaden a selector */
} SimpleSelector;
...
typedef struct {
    ...
    int has_letter_spacing; float letter_spacing; int letter_spacing_em;
    int has_line_clamp; int line_clamp;
    ...
    int has_transform_rotate; float transform_rotate;
    float raw_transform_tx, raw_transform_ty;
    int transform_tx_pct, transform_ty_pct;
    int has_transition; float transition_duration;
    int transition_visual; /* background/border/transform/filter or `all` */
    ...
    /* border-image and its longhands; each part is kept only when given. */
    int has_border_image_source, border_image_none;
    char border_image_path[256];
    int has_border_image_slice;
    float bi_slice[4];
    unsigned char bi_slice_pct;
    int bi_fill;
    int has_border_image_width;
    float bi_width[4];
    ...
    /* Custom properties are resolved per element, through the cascade and
     * inheritance, never against one document-wide table.  custom_props is
     * the rule's own "--name:value\n" list; var_decls its declarations that
     * use var(), as "property\x01raw value\n", substituted for each element
     * the rule matches (owned by the rule text arena). */
    const char* custom_props;
    const char* var_decls;
} StyleRule;
...
/* Text a rule keeps (custom properties, var() declarations); freed with the
 * rules. */
static char** g_rule_texts = NULL;
static int g_rule_text_count = 0, g_rule_text_cap = 0;

static const char* rule_text_keep(const char* s, size_t n) {
    if (!s || !n) return NULL;
    if (g_rule_text_count == g_rule_text_cap) {
        int cap = g_rule_text_cap ? g_rule_text_cap * 2 : 64;
        char** g = (char**)realloc(g_rule_texts, sizeof(char*) * (size_t)cap);
        if (!g) return NULL;
        g_rule_texts = g;
        g_rule_text_cap = cap;
    }
    ...
    memcpy(t, s, n);
    t[n] = '\0';
    g_rule_texts[g_rule_text_count++] = t;
    return t;
}
...
static void rule_bg_layers_release_all(void) {
    ...
    while (g_rule_text_count > 0)
        free(g_rule_texts[--g_rule_text_count]);
}
...
static int font_path_score_symbols(const char* path) {
    const char* solid = luna_strcasestr_local(path, "LunaSymbols-Solid");
    const char* awesome = luna_strcasestr_local(path, "Font Awesome");
    const char* brands = luna_strcasestr_local(path, "Brands");
    if (brands) return -1000;
    if (solid) return 1000;
    if (awesome && (luna_strcasestr_local(path, "Solid") ||
                    luna_strcasestr_local(path, "Free"))) return 500;
    return -1000;
}

static int font_path_score_brands(const char* path) {
    if (luna_strcasestr_local(path, "LunaSymbols-Brands")) return 1000;
    if (luna_strcasestr_local(path, "Font Awesome") &&
        luna_strcasestr_local(path, "Brands")) return 500;
    return -1000;
}
...
/* Internal format for the single-channel glyph atlases.
 *
 * Desktop GL accepts the unsized GL_RED and picks a red format for it.  ES 3
 * does not: its unsized internal formats are only RGBA/RGB/LUMINANCE_ALPHA/
 * LUMINANCE/ALPHA, so GL_RED there names no format at all and the texture ends
 * up incomplete.  Sampling an incomplete texture yields zero, and since the
 * atlas carries glyph *coverage* in that channel, every glyph came out fully
 * transparent — on an ES host the whole UI painted correctly except that no
 * text was ever visible.  GL_R8 is the sized spelling and is valid on both. */
static GLint luna_r8_internal_format(void) {
    return g_luna_gles3 ? GL_R8 : GL_RED;
}
...
static void dyn_flush_atlas(void) {
    ...
    if (!g_dyn_tex) {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, luna_r8_internal_format(),
                     LUNA_DYN_ATLAS_W, LUNA_DYN_ATLAS_H, 0,
...
/* Return glyph advance without allocating/rasterising an atlas entry.
 *
 * Width queries run during intrinsic layout, wrapping, hit-testing and caret
 * placement.  Calling dyn_bake_glyph() from this path made a harmless layout
 * pass upload font-atlas rows to GL.  Periodic status/event wakes could then
 * turn into an unexpected GPU upload and visible hitch.  The dynamic atlas
 * uses this exact stb_truetype metric, so measuring it directly is both
 * side-effect-free and pixel-consistent with the subsequent paint. */
static float glyph_metric_advance(int cp, float css_px) {
    stbtt_fontinfo* finfo = font_for_codepoint(cp);
    int advance = 0, lsb = 0;
    if (!finfo || css_px < 4.0f) return css_px * 0.5f;
    float scale = stbtt_ScaleForMappingEmToPixels(finfo, css_px);
    stbtt_GetCodepointHMetrics(finfo, cp, &advance, &lsb);
    (void)lsb;
...
static float glyph_advance(FontAtlas* atlas, int cp, float css_px) {
    ...
    return glyph_metric_advance(cp, css_px);
}
...
unsigned char* read_file_bytes(const char* filename, long* out_size) {
    ...
    if (g_luna_platform.read_resource) {
        size_t sz = 0;
        unsigned char* p = g_luna_platform.read_resource(filename, &sz);
        if (p) {
    ...
    fclose(file);
    if (got != (size_t)length) { free(buffer); return NULL; }
    buffer[length] = 0;
...
static void parse_transform(const char* val, StyleRule* rule) {
    ...
    if (p && strncmp(p, "none", 4) == 0 &&
        (p[4] == '\0' || p[4] == ';' || isspace((unsigned char)p[4]))) {
        /* `none` is a computed reset, not an absent declaration.  Mark every
           transform component so a later/more-specific rule can override a
           translate/scale/rotate supplied by an earlier rule. */
        rule->has_transform = 1;
        rule->transform_scale = 1.0f;
        rule->has_transform_tx = 1;
        rule->transform_tx = 0.0f;
        rule->raw_transform_tx = 0.0f;
        rule->transform_tx_pct = 0;
        rule->has_transform_ty = 1;
        rule->transform_ty = 0.0f;
        rule->raw_transform_ty = 0.0f;
        rule->transform_ty_pct = 0;
        rule->has_transform_rotate = 1;
        rule->transform_rotate = 0.0f;
        return;
    }
    while (p && *p) {
        while (isspace((unsigned char)*p)) p++;
        if (strncmp(p, "scale(", 6) == 0) {
            ...
            char* endp;
            float angle = strtof(p + 7, &endp);
            if (strncmp(endp, "turn", 4) == 0) angle *= 6.28318530718f;
            else if (strncmp(endp, "rad", 3) != 0) angle *= 0.01745329252f;
            rule->has_transform_rotate = 1;
            rule->transform_rotate = angle;
        }
...
/* Paint-only text mutation for fixed-geometry labels.  This deliberately
 * leaves intrinsic-size and layout caches intact.  It must not be used where
 * the new text is expected to resize this element or any flex/grid ancestor. */
static void set_text_paint_only(int idx, const char* new_text) {
...
/* strstr for an attribute name: the match must start the tag buffer or follow
 * whitespace, so `id="` is not found inside `data-id="`. */
static const char* find_html_attr(const char* tag_buf, const char* needle) {
    const char* p = tag_buf;
    while ((p = strstr(p, needle)) != NULL) {
        if (p == tag_buf || isspace((unsigned char)p[-1])) return p;
        p++;
    }
    return NULL;
}
...
static int extract_html_attr(const char* tag_buf, const char* attr, char* out, int out_len) {
    ...
    const char* p = find_html_attr(tag_buf, needle);
    if (!p) {
        snprintf(needle, sizeof(needle), "%s='", attr);
        p = find_html_attr(tag_buf, needle);
        if (!p) return 0;
        p += strlen(attr) + 2;
...
static void apply_element_inline_style(LunaElement* e, int* am) {
...
int is_visible(int idx) {
    while (idx != -1) {
        if (elements[idx].display_none || elements[idx].dom_detached) return 0;
...
static int element_overflow_visible(int idx) {
    ...
    while (p != -1) {
        ...
        if (overflow_clips(par->overflow_x) || overflow_clips(par->overflow_y)) {
            ...
            float cx = par->x + chain_tx + par->border_width;
            float cy = par->y + chain_ty + par->border_width;
            float cw = par->w - par->border_width * 2.0f;
            float ch = par->h - par->border_width * 2.0f;
...
int simple_selector_matches(SimpleSelector* s, LunaElement* e) {
    ...
    if (s->is_hover && !e->is_hovered) return 0;
    if (s->is_active && !e->is_active) return 0;
    if (s->is_focus && e->id_idx != g_focused_element_idx) return 0;
    if (s->is_focus_visible &&
        (e->id_idx != g_focused_element_idx || !g_focus_via_keyboard)) return 0;
    if (s->is_focus_within && !element_contains_focus(e->id_idx)) return 0;
    if (s->is_root && e->parent_idx != -1) return 0;
    if (s->unsupported_pseudo) return 0;
 ...
 *
 * The monospace role was added to LunaPlatform, but the CSS parser still sent
 * every non-icon family to the regular UI face.  As a result, editors,
 * terminals and any `font-family: monospace` declaration silently changed
 * appearance after the platform/font API update.  Match the generic family
 * and the common concrete monospace names case-insensitively. */
static void apply_font_family_value(StyleRule* rule, const char* val) {
    rule->has_font_face = 1;
    if (val && (luna_strcasestr_local(val, "Luna Symbols") ||
                luna_strcasestr_local(val, "Font Awesome 6 Free") ||
                luna_strcasestr_local(val, "Font Awesome 5 Free"))) {
        rule->font_face = 1;
    } else if (val && (luna_strcasestr_local(val, "Luna Brands") ||
                       luna_strcasestr_local(val, "Font Awesome 6 Brands") ||
                       luna_strcasestr_local(val, "Font Awesome 5 Brands"))) {
        rule->font_face = 2;
    } else if (val && (luna_strcasestr_local(val, "monospace") ||
                       luna_strcasestr_local(val, "SFMono") ||
                       luna_strcasestr_local(val, "SF Mono") ||
                       luna_strcasestr_local(val, "Menlo") ||
                       luna_strcasestr_local(val, "Monaco") ||
                       luna_strcasestr_local(val, "Consolas") ||
                       luna_strcasestr_local(val, "Cascadia Code") ||
                       luna_strcasestr_local(val, "JetBrains Mono") ||
                       luna_strcasestr_local(val, "Fira Code") ||
                       luna_strcasestr_local(val, "Source Code Pro") ||
                       luna_strcasestr_local(val, "IBM Plex Mono") ||
                       luna_strcasestr_local(val, "Ubuntu Mono") ||
                       luna_strcasestr_local(val, "Liberation Mono") ||
                       luna_strcasestr_local(val, "DejaVu Sans Mono"))) {
        rule->font_face = 3;
    } else {
        rule->font_face = 0;
    }
...
static int font_size_keyword_px(const char* tok, float* out_px) {
    ...
    for (size_t i = 0; i < sizeof(kws) / sizeof(kws[0]); i++) {
        if (luna_strcasecmp(tok, kws[i].name) == 0) { *out_px = kws[i].px; return 1; }
    }
    return 0;
}

static int font_token_is_weight(const char* t, int* out_bold) {
    if (luna_strcasecmp(t, "bold") == 0 || luna_strcasecmp(t, "bolder") == 0) {
        if (out_bold) *out_bold = 1;
        return 1;
    }
    if (luna_strcasecmp(t, "normal") == 0 || luna_strcasecmp(t, "lighter") == 0) {
...
static int font_token_is_stretch(const char* t) {
    ...
    for (size_t i = 0; i < sizeof(kws) / sizeof(kws[0]); i++)
        if (luna_strcasecmp(t, kws[i]) == 0) return 1;
...
static float font_parse_line_height(const char* tok) {
    if (!tok || !*tok || luna_strcasecmp(tok, "normal") == 0) return 0.0f;
    char* endp = NULL;
    float lh = strtof(tok, &endp);
    while (endp && isspace((unsigned char)*endp)) endp++;
    if (!endp || endp == tok || lh <= 0.0f) return 0.0f;
    /* Negative values are an internal marker for a font-size multiplier. */
    if (*endp == '\0' || strcmp(endp, "em") == 0) return -lh;
    if (*endp == '%') return -(lh / 100.0f);
    if (strncmp(endp, "rem", 3) == 0) return lh * 16.0f;
...
static void parse_font_shorthand(const char* val, StyleRule* rule) {
    ...
    for (size_t i = 0; i < sizeof(sys) / sizeof(sys[0]); i++) {
        if (luna_strcasecmp(buf, sys[i]) == 0) {
            ...
            rule->font_size = (luna_strcasecmp(buf, "small-caption") == 0 ||
                               luna_strcasecmp(buf, "status-bar") == 0) ? 12.0f : 13.0f;
    ...
    for (int i = 0; i < ntok; i++) {
        ...
        if (font_token_is_size(size_candidate, &px)) {
            ...
            if (pct_only) {
                ...
                for (int j = i + 1; j < ntok; j++) {
                    ...
                    int wb = 0;
                    if (luna_strcasecmp(toks[j], "italic") == 0 || luna_strcasecmp(toks[j], "oblique") == 0 ||
                        luna_strcasecmp(toks[j], "small-caps") == 0 ||
        ...
        int wbold = 0;
        if (luna_strcasecmp(tok, "italic") == 0 || luna_strcasecmp(tok, "oblique") == 0) {
            italic = 1;
        } else if (luna_strcasecmp(tok, "small-caps") == 0) {
            /* font-variant: accepted for CSS parity; no dedicated atlas. */
        } else if (font_token_is_weight(tok, &wbold) && luna_strcasecmp(tok, "normal") != 0) {
            bold = wbold;
        } else if (font_token_is_stretch(tok) && luna_strcasecmp(tok, "normal") != 0) {
            /* font-stretch: accepted / ignored for layout. */
        } else if (luna_strcasecmp(tok, "normal") == 0) {
...
/* border-image-slice: one to four numbers (image pixels) or percentages,
 * and `fill` anywhere.  Returns the number of values read. */
static int parse_border_image_slice(const char* val, StyleRule* rule) {
    float v[4]; unsigned char pct = 0; int n = 0;
    const char* p = val;
    while (*p && n < 4) {
        while (*p == ' ' || *p == '\t') ++p;
        if (!*p || *p == '/') break;
        if (!strncmp(p, "fill", 4)) { rule->bi_fill = 1; p += 4; continue; }
        char* end = NULL;
        float f = strtof(p, &end);
        if (end == p) { while (*p && *p != ' ') ++p; continue; }
        p = end;
        if (*p == '%') { pct |= (unsigned char)(1u << n); ++p; }
        v[n++] = f;
    }
    while (*p == ' ') ++p;
    if (!strncmp(p, "fill", 4)) rule->bi_fill = 1;
    if (!n) return 0;
    /* CSS box expansion: 1 -> all, 2 -> v h, 3 -> t h b. */
    float out[4] = { v[0], n > 1 ? v[1] : v[0], n > 2 ? v[2] : v[0],
                     n > 3 ? v[3] : (n > 1 ? v[1] : v[0]) };
    unsigned char bits[4] = { (unsigned char)(pct & 1),
        (unsigned char)(n > 1 ? (pct >> 1) & 1 : pct & 1),
        (unsigned char)(n > 2 ? (pct >> 2) & 1 : pct & 1),
        (unsigned char)(n > 3 ? (pct >> 3) & 1 : (n > 1 ? (pct >> 1) & 1 : pct & 1)) };
    rule->bi_slice_pct = 0;
    for (int i = 0; i < 4; ++i) {
        rule->bi_slice[i] = out[i];
        rule->bi_slice_pct |= (unsigned char)(bits[i] << i);
    }
    rule->has_border_image_slice = 1;
    return n;
}

/* border-image-width: one to four px lengths (a bare number is a multiple of
 * the border width in CSS; here it is taken as px, the only unit the hosts
 * write).  `auto` is the slice's own size. */
static void parse_border_image_width(const char* val, StyleRule* rule) {
    float v[4]; int n = 0;
    const char* p = val;
    while (*p && n < 4) {
        while (*p == ' ') ++p;
        if (!*p) break;
        if (!strncmp(p, "auto", 4)) { v[n++] = -1.0f; p += 4; continue; }
        char* end = NULL;
        float f = strtof(p, &end);
        if (end == p) break;
        p = end;
    ...
    float out[4] = { v[0], n > 1 ? v[1] : v[0], n > 2 ? v[2] : v[0],
                     n > 3 ? v[3] : (n > 1 ? v[1] : v[0]) };
    for (int i = 0; i < 4; ++i) rule->bi_width[i] = out[i];
    rule->has_border_image_width = 1;
}

/* The ';' that ends the declaration starting at p: one outside parentheses
 * and quotes, so url(data:image/png;base64,...) stays one value. */
static char* css_decl_end(char* p) {
    int depth = 0;
    char quote = 0;
    for (; *p; p++) {
            ...
            if      (strcmp(key, "background-color") == 0) { rule->has_bg = 1; parse_color(val, &rule->bg_r, &rule->bg_g, &rule->bg_b, &rule->bg_a); }
                ...
                if (luna_strcasecmp(val, "none") == 0) {
                    ...
                    rule->bg_image_path[sizeof(rule->bg_image_path) - 1] = '\0';
                     ...
                     strcmp(key, "border-image") == 0) {
                char path[256];
                if (!strncmp(val, "none", 4)) {
                    rule->has_border_image_source = 1;
                    rule->border_image_none = 1;
                } else if (parse_url(val, path, sizeof(path))) {
                    rule->has_border_image_source = 1;
                    rule->border_image_none = 0;
                    snprintf(rule->border_image_path, sizeof(rule->border_image_path), "%s", path);
                }
                if (strcmp(key, "border-image") == 0) {
                    /* url(...) <slice> [fill] [/ <width>] */
                    const char* rest = strchr(val, ')');
                    rest = rest ? rest + 1 : val;
                    parse_border_image_slice(rest, rule);
                    const char* slash = strchr(rest, '/');
                    if (slash) parse_border_image_width(slash + 1, rule);
                }
            }
            else if (strcmp(key, "border-image-slice") == 0) { parse_border_image_slice(val, rule); }
            else if (strcmp(key, "border-image-width") == 0) { parse_border_image_width(val, rule); }
                ...
                rule->has_display = 1;
                rule->inline_level = strcmp(val, "inline") == 0 ? 1 :
                    (strncmp(val, "inline-", 7) == 0) ? 2 : 0;
                if (strcmp(val, "none") == 0) {
                    rule->display_none = 1;
                ...
                } else if (strcmp(val, "-webkit-box") == 0) {
                    /* Legacy WebKit box is used by the line-clamp idiom.
                       Luna lays it out as a block and applies the clamp in the
                       text sizing/paint paths below. */
                    rule->display_none = 0;
                ...
                if (sec > 0.0f) {
                    rule->has_transition = 1;
                    rule->transition_duration = sec;
                    /* The current fast interpolator owns paint colours and
                       transforms, not CSS `opacity` (keyframes own that field).
                       Do not let `transition:opacity` accidentally animate an
                       element's positioning transform and background. */
                    if (strcmp(key, "transition-duration") == 0 ||
                        strstr(val, "all") || strstr(val, "background") ||
                        strstr(val, "color") || strstr(val, "border") ||
                        strstr(val, "transform") || strstr(val, "filter"))
                        rule->transition_visual = 1;
                }
            }
                ...
                rule->line_height = font_parse_line_height(val);
            }
                ...
                rule->letter_spacing_em = 0;
                if (strcmp(val, "normal") == 0) {
                    rule->letter_spacing = 0.0f;
                } else {
                    char* endp = NULL;
                    rule->letter_spacing = strtof(val, &endp);
                    while (endp && isspace((unsigned char)*endp)) endp++;
                    if (endp && strcmp(endp, "em") == 0) rule->letter_spacing_em = 1;
                    else if (endp && strncmp(endp, "rem", 3) == 0)
                        rule->letter_spacing *= 16.0f;
                    else if (!endp || endp == val) rule->letter_spacing = 0.0f;
                }
            }
            else if (strcmp(key, "-webkit-line-clamp") == 0 || strcmp(key, "line-clamp") == 0) {
                rule->has_line_clamp = 1;
                rule->line_clamp = (strcmp(val, "none") == 0) ? 0 : atoi(val);
                if (rule->line_clamp < 0) rule->line_clamp = 0;
            }
            else if (strcmp(key, "-webkit-box-orient") == 0) {
                /* Accepted for browser-compatible `display:-webkit-box` +
                   `-webkit-line-clamp`. Only the vertical text mode is needed. */
            }
...
static void parse_nth_arg(const char* arg, int* out_a, int* out_b) {
    ...
    trim_whitespace(buf);
    *out_a = 0; *out_b = 0;
    if (luna_strcasecmp(buf, "odd") == 0)  { *out_a = 2; *out_b = 1; return; }
    if (luna_strcasecmp(buf, "even") == 0) { *out_a = 2; *out_b = 0; return; }
    char* n = strchr(buf, 'n');
    if (!n) n = strchr(buf, 'N');
...
static int apply_structural_pseudo(SimpleSelector* ss, const CSSSelectorPart* p) {
    ...
    else if (strcmp(p->name, "root") == 0) ss->is_root = 1;
    else if (strcmp(p->name, "before") == 0 || strcmp(p->name, "after") == 0)
        return 1; /* converted to generated pseudo-elements by the caller */
    else return 0;
    return 1;
}
...
static void convert_compound_parts(const CSSCompound* cmp, SimpleSelector* ss) {
    for (int pi = 0; pi < cmp->part_count; pi++) {
        ...
        case CSS_SEL_PSEUDO_CLASS:
            if      (strcmp(p->name, "hover")         == 0) ss->is_hover = 1;
            else if (strcmp(p->name, "active")        == 0) ss->is_active = 1;
            else if (strcmp(p->name, "focus")         == 0) ss->is_focus = 1;
            else if (strcmp(p->name, "focus-visible") == 0) ss->is_focus_visible = 1;
            else if (strcmp(p->name, "focus-within")  == 0) ss->is_focus_within = 1;
            else if (!apply_structural_pseudo(ss, p)) ss->unsupported_pseudo = 1;
            break;
        default: break;
        }
    }
...
static void ingest_parsed_rule(const CSSRule *pr) {
    ...
    /* Custom properties and declarations that use var() are kept as text:
       their values depend on the element they end up applying to. */
    char* texts[2][2] = { { NULL, NULL }, { NULL, NULL } };   /* [importance][custom, var] */
    size_t tlen[2][2] = { { 0, 0 }, { 0, 0 } };
    for (int di = 0; di < pr->decl_count; di++) {
        const CSSDeclaration* d = &pr->decls[di];
        int importance = d->important ? 1 : 0;
        template_used[importance] = 1;
        int kind = strncmp(d->property, "--", 2) == 0 ? 0 : d->variable_source ? 1 : -1;
        if (kind < 0) {
            apply_one_declaration(d->property, d->value, &tmpl[importance]);
            continue;
        }
        const char* raw = d->variable_source ? d->variable_source : d->value;
        size_t add = strlen(d->property) + strlen(raw) + 3;
        char* g = (char*)realloc(texts[importance][kind], tlen[importance][kind] + add);
        if (!g) continue;
        texts[importance][kind] = g;
        tlen[importance][kind] += (size_t)sprintf(g + tlen[importance][kind], "%s%c%s\n",
                                                  d->property, kind == 0 ? ':' : '\x01', raw);
    }
    for (int i = 0; i < 2; i++) {
        tmpl[i].custom_props = rule_text_keep(texts[i][0], tlen[i][0]);
        tmpl[i].var_decls = rule_text_keep(texts[i][1], tlen[i][1]);
        free(texts[i][0]);
        free(texts[i][1]);
    }
    ...
    for (int importance = 0; importance < 2; importance++) {
      ...
      for (int si = 0; si < pr->selector_count && rule_count < MAX_RULES; si++) {
        ...
        for (int a = 0; a < rule.ancestor_count; a++) {
            const SimpleSelector* anc = &rule.ancestors[a];
            if (anc->is_hover)         spec += 10;
            if (anc->is_active)        spec += 10;
            if (anc->is_focus)         spec += 10;
            if (anc->is_focus_visible) spec += 10;
            if (anc->is_focus_within)  spec += 10;
        }
...
static float parse_keyframe_stop_pos(const char* sel) {
    if (!sel) return 0.0f;
    if (luna_strcasecmp(sel, "from") == 0 || luna_strcasecmp(sel, "0%") == 0) return 0.0f;
    if (luna_strcasecmp(sel, "to") == 0 || luna_strcasecmp(sel, "100%") == 0) return 1.0f;
...
/* A dynamic pseudo-class may sit on an ancestor while its declarations target
   a descendant (`.tooltip:hover .tooltip-text`).  Whenever that ancestor's
   state changes, descendants must re-run selector matching as browsers do. */
static void restyle_descendants_of(int ancestor) {
    if (ancestor < 0) return;
    for (int i = 0; i < elem_count; i++)
        if (i != ancestor && is_descendant_of(i, ancestor))
            (void)restyle_element_checked(&elements[i]);
}
 ...
 * by stylesheet rules and the element's own style="" (applied last, so it
 * wins).  am[] records which margins an author set (top right bottom left). */
static void apply_style_rule(LunaElement* e, const StyleRule* r, int* am) {
    ...
    if (r->has_border_image_source) {
        if (r->border_image_none || strcmp(e->border_image_path, r->border_image_path))
            e->border_image_tex = 0;
        e->has_border_image = !r->border_image_none;
        snprintf(e->border_image_path, sizeof(e->border_image_path), "%s",
                 r->border_image_none ? "" : r->border_image_path);
    }
    if (r->has_border_image_slice) {
        for (int k = 0; k < 4; ++k) e->bi_slice[k] = r->bi_slice[k];
        e->bi_slice_pct = r->bi_slice_pct;
        e->bi_fill = r->bi_fill;
    }
    if (r->has_border_image_width)
        for (int k = 0; k < 4; ++k) e->bi_width[k] = r->bi_width[k];
    ...
    if (r->has_margin) {
        if (r->has_margin_top) { am[0] = 1; e->margin_top = r->margin_top; e->margin_top_auto = r->margin_top_auto; }
        if (r->has_margin_right) { am[1] = 1; e->margin_right = r->margin_right; e->margin_right_auto = r->margin_right_auto; }
        if (r->has_margin_bottom) { am[2] = 1; e->margin_bottom = r->margin_bottom; e->margin_bottom_auto = r->margin_bottom_auto; }
        if (r->has_margin_left) { am[3] = 1; e->margin_left = r->margin_left; e->margin_left_auto = r->margin_left_auto; }
    }
    if (r->has_position) {
    ...
    if (r->has_letter_spacing) {
        e->letter_spacing = r->letter_spacing;
        e->letter_spacing_em = r->letter_spacing_em;
    }
    if (r->has_line_clamp) e->line_clamp = r->line_clamp;
    ...
    if (r->has_transform_tx) {
        ...
        /* Dynamic pseudo-class restyles are paint-only.  Resolve against
           the already known border-box now so a focus/hover restyle does
           not reset translateX(-50%) to zero until the next layout. */
        if (e->transform_tx_pct && e->w > 0.0f)
            e->transform_tx = e->raw_transform_tx * e->w;
    }
    if (r->has_transform_ty) {
        ...
        if (e->transform_ty_pct && e->h > 0.0f)
            e->transform_ty = e->raw_transform_ty * e->h;
    }
    if (r->has_transform_rotate) e->transform_rotate = r->transform_rotate;
    if (r->has_transition && r->transition_visual)
        e->anim_speed = 1.0f / r->transition_duration;
...
/* Web-document mode: HTML's default inline boxes and materialized text runs.
 * Off by default so existing Luna UIs keep their all-block layout. */
static int g_luna_web_compat = 0;

static void luna_ua_inline_defaults(LunaElement* e) {
    static const char* inl[] = {
        "a","abbr","b","bdi","bdo","big","cite","code","data","dfn","em","font",
        "i","kbd","label","mark","q","s","samp","small","span","strike","strong",
        "sub","sup","time","tt","u","var","br","wbr","#text",NULL };
    static const char* atomic[] = {
        "img","input","select","textarea","button","svg","canvas","video",
        "audio","iframe","object","embed","progress","meter",NULL };
    for (int i = 0; inl[i]; i++) if (luna_strcasecmp(e->type, inl[i]) == 0) { e->inline_level = 1; return; }
    for (int i = 0; atomic[i]; i++) if (luna_strcasecmp(e->type, atomic[i]) == 0) { e->inline_level = 2; return; }
}

/* An anonymous text run has no style of its own: it is its parent's text.
 * Selectors like `*` still reach it through the cascade, so the box
 * properties they may have set are put back to a bare inline box. */
static void anon_text_fixup(LunaElement* e) {
    const LunaElement* par = (e->parent_idx >= 0 && e->parent_idx < elem_count)
        ? &elements[e->parent_idx] : NULL;
    e->inline_level = 1;
    e->display_mode = DISPLAY_BLOCK;
    e->display_none = 0;
    e->position_mode = 0; e->position_fixed = 0; e->position_sticky = 0;
    e->css_positioned = 0;
    e->has_top = e->has_left = e->has_bottom = e->has_right = 0;
    ...
    e->pad_t = e->pad_r = e->pad_b = e->pad_l = 0.0f;
    e->border_width = 0.0f; e->bd_a = 0.0f;
    e->has_border_top = e->has_border_right = e->has_border_bottom = e->has_border_left = 0;
    e->r = e->g = e->b = e->a = 0.0f;
    e->has_gradient = 0; e->has_bg_image = 0; e->bg_layer_count = 0;
    e->has_shadow = 0; e->shadow_count = 0;
    e->has_css_width = e->has_css_height = 0;
    e->pct_w = e->pct_h = 0;
    e->has_min_width = e->has_min_height = e->has_max_width = e->has_max_height = 0;
    e->opacity = 1.0f;
    e->border_radius = 0.0f;
    e->rad_c[0] = e->rad_c[1] = e->rad_c[2] = e->rad_c[3] = 0.0f;
    e->transform_scale = 1.0f; e->transform_tx = e->transform_ty = 0.0f; e->transform_rotate = 0.0f;
    e->has_css_animation = 0; e->anim_name[0] = '\0';
    e->overflow_x = e->overflow_y = OVERFLOW_VISIBLE;
    e->z_index = 0;
    if (par) {
        e->font_size = par->font_size; e->font_bold = par->font_bold;
        e->font_face = par->font_face; e->font_italic = par->font_italic;
        e->line_height = par->line_height; e->white_space = par->white_space;
        e->letter_spacing = par->letter_spacing; e->text_transform = par->text_transform;
        e->text_decoration = par->text_decoration;
        e->t_r = par->t_r; e->t_g = par->t_g; e->t_b = par->t_b; e->t_a = par->t_a;
        e->cursor_type = par->cursor_type; e->cursor_pointer = par->cursor_pointer;
    }
}
...
void update_element_style(LunaElement* e) {
    ...
    /* border-image comes from the sheet alone; start each cascade without one. */
    {
        const int had = e->has_border_image;
        char prev[sizeof e->border_image_path];
        memcpy(prev, e->border_image_path, sizeof prev);
        e->has_border_image = 0; e->bi_fill = 0; e->bi_slice_pct = 0;
        for (int k = 0; k < 4; ++k) { e->bi_slice[k] = 0.0f; e->bi_width[k] = -1.0f; }
        /* Keep the texture when the cascade gives the same image back. */
        if (!had) e->border_image_tex = 0;
        (void)prev;
    }
     ...
     * recalculates the extents and clamps the offsets after a real layout. */
    e->scroll_smooth = 0;
    ...
    if (e->parent_idx >= 0 && e->parent_idx < elem_count) {
        ...
        /* letter-spacing inherits as its computed length, not as the parent's
           original em token.  line-clamp is intentionally not inherited. */
        e->letter_spacing = parent->letter_spacing;
        e->letter_spacing_em = 0;
    ...
    /* Web documents (luna_set_web_compat): the HTML UA stylesheet's outer
     * display types.  Luna's own UIs predate inline layout and keep every
     * element block-level unless their CSS says otherwise. */
    if (g_luna_web_compat) luna_ua_inline_defaults(e);
    e->has_text_shadow = 0;
    ...
    e->transform_rotate = 0.0f;
    e->transform_tx_pct = 0; e->transform_ty_pct = 0;
    e->raw_transform_tx = 0.0f; e->raw_transform_ty = 0.0f;
    /* CSS changes are immediate unless the cascade supplies a transition.
       The previous implicit 14 Hz exponential easing faded every freshly
       styled background/border in from parser defaults, which both differed
       from browsers and kept otherwise static documents repainting. */
    e->anim_speed = 0.0f;
    ...
    apply_element_inline_style(e, am);
    ...
    if (strcmp(e->type, "h1") == 0 || strcmp(e->type, "h2") == 0 ||
        ...
        if (!am[0]) e->margin_top = e->font_size * em;
        if (!am[2]) e->margin_bottom = e->font_size * em;
    }

    if (e->letter_spacing_em) {
        e->letter_spacing *= e->font_size;
        e->letter_spacing_em = 0;
    }
    ...
    if (e->anon_text) anon_text_fixup(e);
...
static int tag_attr_equals(const char* tag_buf, const char* key, const char* val) {
    ...
    return luna_strcasecmp(got, val) == 0;
}
...
static int tag_is_stylesheet_link(const char* tag_buf) {
    if (!tag_attr_equals(tag_buf, "rel", "stylesheet")) {
        ...
        return luna_strcasestr_local(rval, "stylesheet") != NULL;
    }
    return 1;
}
...
static unsigned g_dom_uid_seq = 0;
static int g_dom_uid_dirty = 1;

/* The source attributes of a start tag: everything after the tag name. */
static char* dom_attrs_from_tag(const char* tag_buf) {
    const char* a = tag_buf;
    while (*a && !isspace((unsigned char)*a) && *a != '/') a++;
    size_t n = strlen(a);
    while (n > 0 && (a[n - 1] == '/' || isspace((unsigned char)a[n - 1]))) n--;
...
/* Web mode: a text run between element siblings becomes anonymous inline
 * "#text" elements (split to fit LunaElement::text).  Whitespace collapses to
 * single spaces; a run that is only whitespace just marks the next box. */
static void web_text_run(int parent, const char* start, const char* end, int* pending_ws) {
    if (!start || !end || end <= start || parent < 0 || parent >= elem_count) return;
    char buf[LUNA_UI_TEXT_CAP];
    const char* p = start;
    int lead = isspace((unsigned char)*p);
    while (p < end && isspace((unsigned char)*p)) p++;
    if (p >= end) { *pending_ws = 1; return; }
    int ws = lead || *pending_ws;
    while (p < end && elem_count < MAX_ELEMENTS) {
        size_t n = 0;
        int space = 0;
        /* Fill up to the cap, collapsing whitespace, cutting at a space (or
           a UTF-8 boundary) when the run is longer. */
        size_t cut = 0;
        const char* cut_p = NULL;
        while (p < end && n < sizeof(buf) - 8) {
            unsigned char c = (unsigned char)*p;
            if (isspace(c)) {
                if (!space) { cut = n; cut_p = p; buf[n++] = ' '; }
                space = 1;
                p++;
                continue;
            }
        ...
        if (p < end && cut_p) { n = cut; p = cut_p; }
        else if (p < end) while (n > 0 && ((unsigned char)buf[n - 1] & 0xC0) == 0x80) { n--; p--; }
        if (p < end) while (n > 0 && ((unsigned char)buf[n - 1] & 0xC0) == 0xC0) { n--; p--; }
        int trail = n > 0 && buf[n - 1] == ' ';
        while (n > 0 && buf[n - 1] == ' ') n--;
        buf[n] = '\0';
        decode_html_entities(buf);
        while (p < end && isspace((unsigned char)*p)) { p++; trail = 1; }
        if (!buf[0]) break;
        if (!luna_ensure_element_capacity(elem_count + 1)) return;
        LunaElement* t = &elements[elem_count];
        memset(t, 0, sizeof(*t));
        t->id_idx = elem_count;
        t->parent_idx = parent;
        t->tabindex = -2;
        t->aria_expanded = -1;
        t->cur_scale = 1.0f;
        t->anim_start_time = -1.0;
        strncpy(t->type, "#text", sizeof(t->type) - 1);
        snprintf(t->text, sizeof(t->text), "%s", buf);
        t->anon_text = 1;
        t->ws_before = ws;
        t->dom_uid = ++g_dom_uid_seq;
        update_element_style(t);
        t->cur_r = t->r; t->cur_g = t->g; t->cur_b = t->b; t->cur_a = t->a;
        elem_count++;
        ws = trail;
    }
    *pending_ws = ws;
...
static void parse_html_into(const char* html, int root_parent) {
    ...
    while (*p && elem_count < MAX_ELEMENTS) {
        ...
        if (tag_start[1] == '/') {
            ...
            current_parent = (stack_ptr > 0) ? parent_stack[stack_ptr - 1] : root_parent;
            const char* gt = strchr(tag_start, '>');
            ...
            if (gt && current_parent >= 0) {
                ...
                if (text_end && g_luna_web_compat)
                    web_text_run(current_parent, gt + 1, text_end, &pending_ws);
                else if (text_end) append_direct_text(current_parent, gt + 1, text_end);
            }
            p = gt ? gt + 1 : tag_start + strlen(tag_start);
            continue;
        }
        ...
        if (luna_strcasecmp(type, "meta") == 0) {
            p = tag_end + 1;
            continue;
        }

        if (luna_strcasecmp(type, "title") == 0) {
            const char* text_start = tag_end + 1;
            const char* close_title = luna_strcasestr_local(text_start, "</title>");
        ...
        if (luna_strcasecmp(type, "style") == 0) {
            const char* css_start = tag_end + 1;
            const char* close_pos = luna_strcasestr_local(css_start, "</style>");
        ...
        if (luna_strcasecmp(type, "script") == 0) {
            char close_tag[40]; snprintf(close_tag, sizeof(close_tag), "</%s", type);
            const char* close_pos = luna_strcasestr_local(tag_end + 1, close_tag);
            const char* gt2 = close_pos ? strchr(close_pos, '>') : NULL;
            p = gt2 ? gt2 + 1 : tag_end + 1;
            continue;
        }
        ...
        if (luna_strcasecmp(type, "body") == 0 && root_parent < 0) {
            int existing = -1;
            for (int i = 0; i < elem_count; i++)
            ...
            if (existing == -1 && elem_count < MAX_ELEMENTS) {
                ...
                elements[bi].dom_uid = ++g_dom_uid_seq;
                elements[bi].dom_attrs = dom_attrs_from_tag(tag_buf);
                char bcls[96] = {0};
                extract_html_attr(tag_buf, "class", bcls, sizeof(bcls));
                snprintf(elements[bi].class_name, sizeof(elements[bi].class_name), "%s", bcls);
                extract_html_attr(tag_buf, "id", elements[bi].id, sizeof(elements[bi].id));
                if (extract_html_attr(tag_buf, "style", elements[bi].inline_style,
                                      sizeof(elements[bi].inline_style)))
                    elements[bi].has_inline_style = elements[bi].inline_style[0] != '\0';
        ...
        if (is_ignored_element(type)) {
            if (!is_void_element(type) && !is_self_closing &&
                luna_strcasecmp(type, "html") != 0 && luna_strcasecmp(type, "head") != 0 &&
                luna_strcasecmp(type, "body") != 0) {
        ...
        /* Web mode keeps text on the element only when it is the element's
           whole content; otherwise it is a run among its children (or, after
           a void element, among its siblings). */
        int web_run = 0;
        if (g_luna_web_compat && next_tag) {
            size_t tl = strlen(type);
            int own_close = next_tag[1] == '/' && strncasecmp(next_tag + 2, type, tl) == 0 &&
                            (next_tag[2 + tl] == '>' || isspace((unsigned char)next_tag[2 + tl]));
            web_run = is_void_element(type) || is_self_closing || !own_close ||
                      (int)(next_tag - tag_end - 1) >= (int)sizeof(text) - 1;
        }
        if (next_tag && !web_run) {
        ...
        if (luna_strcasecmp(type, "input") == 0 || luna_strcasecmp(type, "textarea") == 0) {
            e.is_input = 1;
            e.input_multiline = (luna_strcasecmp(type, "textarea") == 0) ? 1 : 0;
            e.caret = 0;
            e.cursor_type = 2; /* text */
            if (e.tabindex == -2) e.tabindex = 0;
            char itype[32] = {0};
            extract_html_attr(tag_buf, "type", itype, sizeof(itype));
            if (itype[0] && luna_strcasecmp(itype, "password") == 0) e.input_password = 1;
            if (itype[0] && (luna_strcasecmp(itype, "submit") == 0 || luna_strcasecmp(itype, "button") == 0 ||
                             luna_strcasecmp(itype, "checkbox") == 0 || luna_strcasecmp(itype, "radio") == 0 ||
                             luna_strcasecmp(itype, "file") == 0 || luna_strcasecmp(itype, "hidden") == 0 ||
                             luna_strcasecmp(itype, "image") == 0 || luna_strcasecmp(itype, "reset") == 0)) {
        ...
        if (!luna_ensure_element_capacity(elem_count + 1)) break;
        e.dom_attrs = dom_attrs_from_tag(tag_buf);
        ...
        elem_count++;
        p = tag_end + 1;
        if (web_run && next_tag) web_text_run(current_parent, tag_end + 1, next_tag, &pending_ws);
    }
...
void parse_html(const char* html) { parse_html_into(html, -1); }
...
/* CSS `normal` is font-metric dependent.  1.2 closely matches the browser
 * line boxes used by the bundled Inter/fallback stack and avoids the previous
 * 1.5x + 12px minimum inflation for small labels. */
static float css_normal_line_height(float font_size) {
    if (font_size <= 0.0f) font_size = 16.0f;
    return font_size * 1.2f;
}

float luna_line_height(float css_px) { return css_normal_line_height(css_px); }

float luna_measure_text(const char* text, float css_px, int bold) {
    if (!text || css_px <= 0.0f) return 0.0f;
    const int saved_bold = g_font_bold_hint;
    ...
    while (*p) {
        ...
        if (cp == '\n' || cp == '\r') continue;
        w += glyph_metric_advance(cp, css_px);
    }
    g_font_bold_hint = saved_bold;
    return w;
}

static float css_used_line_height(const LunaElement* e) {
    float lh = e ? e->line_height : 0.0f;
    float fs = (e && e->font_size > 0.0f) ? e->font_size : 16.0f;
    if (lh < 0.0f) lh = -lh * fs;
...
static float flow_text_available_width(const LunaElement* e) {
    float outer = 0.0f;
    if (!e) return 0.0f;
    if (e->has_css_width && !e->pct_w) {
        outer = css_outer_width(e, e->css_width);
    } else if (e->parent_idx >= 0 && e->parent_idx < elem_count) {
        const LunaElement* p = &elements[e->parent_idx];
        outer = p->w - p->pad_l - p->pad_r - p->border_width * 2.0f
              - e->margin_left - e->margin_right;
    } else {
        outer = e->w;
    }
    outer -= e->pad_l + e->pad_r + e->border_width * 2.0f;
    return outer > 0.0f ? outer : 0.0f;
}

/* Used by headless/layout-only builds where no atlas is available.  It is not
 * a painter approximation: the normal runtime path below uses the real glyph
 * advances.  This fallback still gives auto-height text browser-like wrapping. */
static int flow_approx_text_lines(const LunaElement* e, float box_w, int max_lines) {
    const char* p;
    float advance;
    int cols, lines = 1, col = 0, last_space = -1;
    if (!e || !e->text[0]) return 0;
    if (e->white_space == 1 || box_w <= 0.0f) return 1;
    advance = e->font_size * 0.55f + e->letter_spacing;
    if (advance < 1.0f) advance = 1.0f;
    cols = (int)floorf(box_w / advance);
    if (cols < 1) cols = 1;
    for (p = e->text; *p && lines < max_lines; p++) {
        if (*p == '\n' || *p == '\r') {
            lines++; col = 0; last_space = -1; continue;
        }
        if (*p == ' ' || *p == '\t') last_space = col;
        col++;
        if (col > cols) {
            lines++;
            col = (last_space >= 0) ? col - last_space - 1 : 1;
            last_space = -1;
        }
    }
...
static float flow_leaf_text_height(LunaElement* e) {
    float lh, box_w;
    int max_lines, lines;
    if (!e || !e->text[0])
        return e ? e->pad_t + e->pad_b + e->border_width * 2.0f : 0.0f;
    lh = css_used_line_height(e);
    box_w = flow_text_available_width(e);
    max_lines = e->line_clamp > 0 ? e->line_clamp : 4096;
    if (e->white_space == 1) max_lines = 1;
    if (font_loaded && box_w > 0.0f) {
        FontAtlas* atlas = get_atlas(e->font_size, e->font_bold, NULL);
        text_metrics_begin(e->font_size, e->font_bold, e->font_face, atlas);
        g_text_letter_spacing = e->letter_spacing;
        lines = count_text_lines(atlas, e->text, box_w, lh, max_lines,
                                 e->white_space,
                                 (e->text_overflow || e->line_clamp > 0),
                                 e->overflow_wrap);
        g_text_letter_spacing = 0.0f;
        text_metrics_end();
    } else {
        lines = flow_approx_text_lines(e, box_w, max_lines);
    }
    if (lines < 1) lines = 1;
    return (float)lines * lh + e->pad_t + e->pad_b + e->border_width * 2.0f;
}

/* ------------------------------------------------------------------------
 * Inline formatting context.
 *
 * A block container whose in-flow children include inline-level boxes lays
 * them out on line boxes: greedily, left to right, a new line when the next
 * box does not fit.  Each box is atomic (an inline element whose content is
 * wider than the line takes a whole line and wraps inside itself), boxes sit
 * on the line's bottom edge, a block-level child or <br> ends the line.  One
 * pass, no allocation; the same routine measures (place == 0) and places.
 * ------------------------------------------------------------------------ */
static float intrinsic_content_width(LunaElement* e);
static float flow_content_height(LunaElement* e);

static int has_inline_children(int idx) {
    const LunaElement* cont = &elements[idx];
    if (cont->display_mode == DISPLAY_FLEX || cont->display_mode == DISPLAY_GRID) return 0;
    for (int c = idx + 1; c < elem_count; c++) {
        const LunaElement* ch = &elements[c];
        if (ch->parent_idx != idx || !ch->inline_level || ch->display_none) continue;
...
static float inline_item_height(LunaElement* ch) {
    ...
    if (ch->has_css_height && !ch->pct_h && ch->inline_level == 2)
        h = css_outer_height(ch, ch->css_height);
    else h = flow_content_height(ch);
    if (ch->is_input && !ch->has_css_height && !ch->pct_h) {
        float lines = ch->input_multiline ? 2.0f : 1.0f;
        float min_h = css_used_line_height(ch) * lines + ch->pad_t + ch->pad_b + ch->border_width * 2.0f;
        if (h < min_h) h = min_h;
    }
    if (ch->has_min_height) { float m = css_outer_height(ch, ch->css_min_height); if (h < m) h = m; }
    if (ch->has_max_height && !ch->max_height_pct) { float m = css_outer_height(ch, ch->css_max_height); if (h > m) h = m; }
    return h;
}

static float inline_flow(int idx, float inner_w, int place) {
    LunaElement* cont = &elements[idx];
    const float x0 = cont->border_width + cont->pad_l;
    const float y0 = cont->border_width + cont->pad_t;
    const float space_w = (cont->font_size > 0 ? cont->font_size : 16.0f) * 0.28f;
    int line[MAX_ELEMENTS];
    int n = 0;
    float line_x = 0.0f, line_h = 0.0f, y = 0.0f;
    for (int c = idx + 1; c <= elem_count; c++) {
        LunaElement* ch = c < elem_count ? &elements[c] : NULL;
        int end = !ch;
        int is_block = 0, is_br = 0;
        if (ch) {
            if (ch->parent_idx != idx || !is_visible(c) || ch->position_fixed) continue;
            if (ch->position_mode == POS_ABSOLUTE || (ch->css_positioned & 3)) continue;
            ch->flow_child = 1;
            is_br = strcmp(ch->type, "br") == 0;
            is_block = !ch->inline_level;
        }
        float w = 0.0f, h = 0.0f, sp = 0.0f;
        if (ch && !is_block && !is_br) {
            if (ch->pct_w) w = inner_w * ch->raw_w;
            else if (ch->has_css_width && ch->inline_level == 2) w = css_outer_width(ch, ch->css_width);
            else w = intrinsic_content_width(ch);
            /* Text controls default to the UA's size="20" / cols="20" box,
               whatever they contain. */
            if (ch->is_input && !ch->has_css_width && !ch->pct_w) {
                float fs = ch->font_size > 0 ? ch->font_size : 13.333f;
                w = fs * 10.25f + ch->pad_l + ch->pad_r + ch->border_width * 2.0f;
            }
            float avail = inner_w - ch->margin_left - ch->margin_right;
        ...
        /* Close the current line: at the end, before a block, at <br>, or
           when the next box does not fit. */
        if (n > 0 && (end || is_block || (line_x + sp + outer_w > inner_w + 0.5f && !is_br))) {
            if (place) {
                float shift = 0.0f;
                if (cont->text_align == 1) shift = (inner_w - line_x) * 0.5f;
                else if (cont->text_align == 2) shift = inner_w - line_x;
                if (shift < 0.0f) shift = 0.0f;
                for (int k = 0; k < n; k++) {
                    LunaElement* it = &elements[line[k]];
                    float oh = it->margin_top + it->h + it->margin_bottom;
                    it->rel_x += shift;
                    it->rel_y = y0 + y + (line_h - oh);
                }
            }
            y += line_h;
            n = 0; line_x = 0.0f; line_h = 0.0f;
            sp = 0.0f;
        }
        if (end) break;
        if (is_br) {
            float lh = css_used_line_height(cont);
            if (n == 0) y += lh;           /* an empty line still has height */
            else { if (line_h < lh) line_h = lh; }
            if (n > 0) {
                /* finish this line now */
                if (place) for (int k = 0; k < n; k++) {
                    LunaElement* it = &elements[line[k]];
                    float oh = it->margin_top + it->h + it->margin_bottom;
                    float shift = cont->text_align == 1 ? (inner_w - line_x) * 0.5f
                                : cont->text_align == 2 ? inner_w - line_x : 0.0f;
                    if (shift < 0.0f) shift = 0.0f;
                    it->rel_x += shift;
                    it->rel_y = y0 + y + (line_h - oh);
                }
            ...
            if (place) { ch->rel_x = x0; ch->rel_y = y0 + y; ch->w = 0.0f; ch->h = 0.0f; }
            continue;
        }
        if (is_block) {
            /* A block among inline content: its own line, full width. */
            if (ch->pct_w) ch->w = inner_w * ch->raw_w;
            else if (!ch->has_css_width) ch->w = inner_w - ch->margin_left - ch->margin_right;
            if (ch->w < 0.0f) ch->w = 0.0f;
            if (!ch->has_css_height && !ch->pct_h) ch->h = flow_content_height(ch);
            if (place) { ch->rel_x = x0; ch->rel_y = y0 + y; }
            y += ch->margin_top + ch->h + ch->margin_bottom;
            continue;
        }
        ch->h = h;
        if (place) ch->rel_x = x0 + line_x + sp;
        line_x += sp + outer_w;
        float oh = ch->margin_top + h + ch->margin_bottom;
        if (oh > line_h) line_h = oh;
        if (n < MAX_ELEMENTS) line[n++] = c;
    }
    return y;
}
...
static float flow_content_height(LunaElement* e) {
    ...
    if (idx >= 0 && idx < elem_count && has_inline_children(idx)) {
        float inner_w = e->w - e->pad_l - e->pad_r - e->border_width * 2.0f;
        if (inner_w < 0.0f) inner_w = 0.0f;
        return inline_flow(idx, inner_w, 0) + e->pad_t + e->pad_b + e->border_width * 2.0f;
    }
    ...
    if (n > 0) {
        ...
        if (row && e->text[0]) {
            float text_lh = css_used_line_height(e);
    ...
    return flow_leaf_text_height(e);
}
...
static float intrinsic_content_width(LunaElement* e) {
    ...
    float result;
    int n = 0;
    float total = 0.0f, maxw = 0.0f, inline_run = 0.0f;
    if (e->has_css_width && !e->pct_w) {
        result = e->css_width;
        goto done;
    }
    ...
    for (int c = 0; c < elem_count; c++) {
        ...
        if (ch->inline_level) {
            ...
            if (inline_run > maxw && e->display_mode != DISPLAY_FLEX && e->display_mode != DISPLAY_GRID)
                maxw = inline_run;
        } else inline_run = 0.0f;
        if (strcmp(ch->type, "br") == 0) inline_run = 0.0f;
        n++;
    }
    if (n > 0) {
        int row = (e->display_mode == DISPLAY_FLEX && e->flex_direction == FLEX_DIR_ROW);
...
static void layout_block_container(int container_idx) {
    ...
    if (has_inline_children(container_idx)) {
        y += inline_flow(container_idx, inner_w, 1);
        have_previous = 1;
        cont = &elements[container_idx];
        goto flowed;
    }
...
flowed:
    ...
    if (!cont->has_css_height && !cont->pct_h &&
        ...
        /* `y` has only advanced past in-flow *element* children.  A box whose
         * content is its own text has none of those, so shrink-wrapping to it
         * leaves the box at padding height — for an absolutely positioned
         * label with no padding, exactly zero.  Nothing else goes wrong: the
         * box keeps its width and position, and the text is simply clipped
         * away, so the symptom is a correctly laid out UI with no text in it.
         * CSS puts that text in an anonymous block, so its line box counts. */
        if (!have_previous && cont->text[0]) {
            float text_h = flow_leaf_text_height(cont);
            if (text_h > auto_h) auto_h = text_h;
        }
        cont->h = auto_h;
    ...
    for (int i = 0; i < elem_count; i++) {
        ...
        if (e->has_css_width && !e->pct_w) {
            if (e->box_sizing == BOX_CONTENT)
                ...
                e->w = e->css_width;
        } else if (!e->pct_w && !(e->has_left && e->has_right) &&
                   (e->position_mode == POS_ABSOLUTE || e->position_fixed)) {
            /* CSS 2.1 shrink-to-fit width for absolutely/fixed positioned
             * boxes whose width is auto (also how fit-content is represented
             * by the parser).  Keeping the bootstrap width here left a flex
             * dock with a ~100px background while its children overflowed far
             * to the right, so translateX(-50%) centered the wrong rectangle. */
            float preferred = intrinsic_content_width(e);
            float minimum = min_content_width(e);
            float available = parent_w - e->margin_left - e->margin_right;
            /* The common fit-content centering idiom uses left:50% together
             * with translateX(-50%).  Its available width spans both sides of
             * the center; treating left as a hard left inset clamps the dock at
             * half the viewport and makes the last items overflow its paint. */
            int centered_x = e->pct_left && fabsf(e->raw_left - 0.5f) < 0.0001f &&
                             e->transform_tx_pct &&
                             fabsf(e->raw_transform_tx + 0.5f) < 0.0001f;
            if (e->has_left && !centered_x)
                available -= e->pct_left
                    ? parent_w * e->raw_left + e->raw_left_off
                    : e->rel_x;
            if (e->has_right && !centered_x)
                available -= e->pct_right
                    ? parent_w * e->raw_right + e->raw_right_off
                    : e->right_val;
            if (available < 0.0f) available = 0.0f;
            e->w = preferred < available ? preferred : available;
            if (e->w < minimum) e->w = minimum;
        }
        ...
        if (!e->pos_overridden_x) {
            ...
            if (e->has_left && e->has_right && !e->has_css_width && !e->pct_w) {
                ...
                e->rel_x = lv;
                /* CSS 10.3.7: with width:auto and both offsets given, the
                 * margins are inside the two insets --
                 *   left + margin-left + width + margin-right + right = cb
                 * -- so the box is narrower by them, it is not the full span
                 * shifted sideways.  Leaving them out made `left:0;right:0`
                 * plus a margin a full-width box pushed right by margin-left,
                 * which is how a notification card with `margin:20px 57px`
                 * hung 57px past the right edge of its window. */
                e->w = parent_w - lv - rv - e->margin_left - e->margin_right;
                if (e->w < 0.0f) e->w = 0.0f;
        ...
        if (!e->pos_overridden_y) {
            ...
            if (e->has_top && e->has_bottom && !e->has_css_height && !e->pct_h) {
                ...
                e->rel_y = tv;
                /* CSS 10.6.4, the vertical half of the rule above. */
                e->h = parent_h - tv - bv - e->margin_top - e->margin_bottom;
                if (e->h < 0.0f) e->h = 0.0f;
...
void update_layout_pass(void) {
    ...
    for (int i = 0; i < elem_count; i++) {
        LunaElement* e = &elements[i];
        if (e->transform_tx_pct) {
            e->transform_tx = e->raw_transform_tx * e->w;
            if (e->anim_speed <= 0.0f) e->cur_tx = e->transform_tx;
        }
        if (e->transform_ty_pct) {
            e->transform_ty = e->raw_transform_ty * e->h;
            if (e->anim_speed <= 0.0f) e->cur_ty = e->transform_ty;
        }
    }
...
static void rc_fill_slow(int i) {
    ...
    for (int p = elements[i].parent_idx; p != -1; p = elements[p].parent_idx) {
        ...
        float pw = viewport_body ? window_width :
            par->w - par->border_width * 2.0f;   /* padding box */
        float ph = viewport_body ? window_height :
            par->h - par->border_width * 2.0f;
        float px = viewport_body ? 0.0f : par->x + par->border_width;
        float py = viewport_body ? 0.0f : par->y + par->border_width;
...
static int update_animations_internal(double dt, const int* roots, int nroots,
    ...
    while (pos < g_visual_active_count) {
        ...
        if (element_visual_unsettled(e, target_scale, target_a)) {
            /* No CSS transition means snap to the computed style.  Internal
               overlay nodes retain anim_speed=14 and author transitions set
               their own positive rate. */
            float factor = e->anim_speed <= 0.0f ? 1.0f : default_factor;
...
static int LUNA_UNUSED effective_z_index(int idx) {
...
static int bake_font_atlas(const unsigned char* ttf_buffer, FontAtlas* atlas, float font_size) {
    ...
    /* An atlas that rasterized to nothing produces a UI with every box in the
     * right place and no text at all, and it does it without a single GL
     * error — worth naming rather than leaving to be rediscovered. */
    {
        size_t n = (size_t)LUNA_ASCII_ATLAS_SIZE * (size_t)LUNA_ASCII_ATLAS_SIZE;
        size_t ink = 0;
        for (size_t k = 0; k < n; ++k) if (temp_bitmap[k]) { ink = 1; break; }
        if (!ink)
            fprintf(stderr, "[luna-ui] glyph atlas is blank at %.1fpx — the "
                            "font rasterized no coverage\n", font_size);
    }
...
static int is_font_file(const char* name) {
    ...
    return (luna_strcasecmp(ext, ".ttf") == 0 ||
            luna_strcasecmp(ext, ".otf") == 0 ||
            luna_strcasecmp(ext, ".ttc") == 0 ||
            luna_strcasecmp(ext, ".otc") == 0);
}
...
static void fpl_add(FontPathList* fpl, const char* path) {
    ...
    if (fpl->count >= fpl->cap) {
        int new_cap = fpl->cap ? fpl->cap * 2 : 64;
        char** grown = (char**)realloc(fpl->paths, sizeof(char*) * (size_t)new_cap);
        if (!grown) return;
        fpl->paths = grown;
        fpl->cap = new_cap;
    }
    copy = luna_strdup_local(path);
    if (copy) fpl->paths[fpl->count++] = copy;
}
...
static unsigned char* luna_load_font_role(int role, long* out_size) {
    size_t n = 0;
    unsigned char* p = NULL;
    if (g_luna_platform.load_font) p = g_luna_platform.load_font(role, &n);
    if (out_size) *out_size = (long)n;
    return p;
}

static unsigned char* luna_try_font_candidates(const char* const* paths,
                                                size_t count, long* out_size,
                                                const char* label) {
    for (size_t i = 0; i < count; ++i) {
        if (!paths[i] || !paths[i][0]) continue;
        unsigned char* p = read_file_bytes(paths[i], out_size);
        if (p) {
            fprintf(stderr, "[luna-ui] %s loaded: %s\n", label, paths[i]);
            return p;
        }
    }
    return NULL;
...
void init_font() {
    ...
    long cjk_sz = 0, mono_sz = 0, icon_sz = 0, brand_sz = 0, bold_sz = 0;

#if LUNA_UI_ENABLE_FONT_SCAN
    scan_fonts_dir("/usr/share/fonts", &reg, &bold_list);
    scan_fonts_dir("/usr/local/share/fonts", &reg, &bold_list);
#endif
    scan_fonts_dir("ui/fonts", &reg, &bold_list);
    scan_fonts_dir("skins/fonts", &reg, &bold_list);

    reg_buf = luna_load_font_role(LUNA_FONT_REGULAR, &g_font_ttf_sz);
    g_bold_font_ttf = luna_load_font_role(LUNA_FONT_BOLD, &bold_sz);
    g_cjk_font_ttf = luna_load_font_role(LUNA_FONT_CJK, &cjk_sz);
    g_mono_font_ttf = luna_load_font_role(LUNA_FONT_MONO, &mono_sz);
    g_icon_font_ttf = luna_load_font_role(LUNA_FONT_SYMBOLS, &icon_sz);
    g_brand_font_ttf = luna_load_font_role(LUNA_FONT_BRANDS, &brand_sz);

    if (!reg_buf) {
        const char* env_ui = getenv("LUNA_FONT_REGULAR");
        if (env_ui && env_ui[0]) reg_buf = read_file_bytes(env_ui, &g_font_ttf_sz);
    }
    if (!reg_buf) {
    ...
    if (!g_bold_font_ttf) {
        static const char* const cands[] = {
            "fonts/Inter-Bold.ttf", "ui/fonts/Inter-Bold.ttf",
            "skins/fonts/web/Inter-Bold.ttf",
            "apps/luna-shell/skins/fonts/web/Inter-Bold.ttf",
            "../skins/fonts/web/Inter-Bold.ttf"
        };
        g_bold_font_ttf = luna_try_font_candidates(cands, sizeof(cands)/sizeof(cands[0]),
                                                   &bold_sz, "bold font");
    }
    if (!g_bold_font_ttf)
    ...
    if (!g_cjk_font_ttf) {
        const char* env = getenv("LUNA_FONT_CJK");
        if (env && env[0]) g_cjk_font_ttf = read_file_bytes(env, &cjk_sz);
    }
    if (!g_cjk_font_ttf) g_cjk_font_ttf = try_load_font_list(&reg, font_path_score, &cjk_sz);

    if (!g_mono_font_ttf) {
        const char* env = getenv("LUNA_FONT_MONO");
        if (env && env[0]) g_mono_font_ttf = read_file_bytes(env, &mono_sz);
    }
    if (!g_mono_font_ttf) g_mono_font_ttf = try_load_font_list(&reg, font_path_score_mono, &mono_sz);

    if (!g_icon_font_ttf) {
        const char* env = getenv("LUNA_FONT_ICONS");
        const char* const cands[] = {
            env && env[0] ? env : NULL,
            "fonts/LunaSymbols-Solid.otf", "ui/fonts/LunaSymbols-Solid.otf",
            "skins/fonts/LunaSymbols-Solid.otf", "../skins/fonts/LunaSymbols-Solid.otf",
            ...
            "/usr/local/share/fonts/LunaSymbols-Solid.otf",
            "/usr/local/share/fonts/luna/LunaSymbols-Solid.otf"
        };
        g_icon_font_ttf = luna_try_font_candidates(cands, sizeof(cands)/sizeof(cands[0]),
                                                   &icon_sz, "symbol font");
    }
    if (!g_icon_font_ttf)
        g_icon_font_ttf = try_load_font_list(&reg, font_path_score_symbols, &icon_sz);

    if (!g_brand_font_ttf) {
        const char* env = getenv("LUNA_FONT_BRANDS");
        const char* const cands[] = {
            env && env[0] ? env : NULL,
            "fonts/LunaSymbols-Brands.otf", "ui/fonts/LunaSymbols-Brands.otf",
            "skins/fonts/LunaSymbols-Brands.otf", "../skins/fonts/LunaSymbols-Brands.otf",
            ...
            "/usr/local/share/fonts/LunaSymbols-Brands.otf",
            "/usr/local/share/fonts/luna/LunaSymbols-Brands.otf"
        };
        g_brand_font_ttf = luna_try_font_candidates(cands, sizeof(cands)/sizeof(cands[0]),
                                                    &brand_sz, "brand font");
    }
    if (!g_brand_font_ttf)
    ...
    if (reg_buf) {
        ...
        fprintf(stderr, "[luna-ui] warning: no regular font available\n");
    }
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * 4 * 6 * LUNA_TEXT_BATCH_GLYPHS,
...
/* One piece of a nine-slice: the source rectangle (image pixels, top-left
 * origin) drawn stretched into the destination rectangle, through the image
 * shader's image rect — the quad is the piece, and the image rect is placed
 * so exactly the source rectangle lands in it. */
static void draw_image_piece(float dx, float dy, float dw, float dh,
                             float sx, float sy, float sw, float sh,
                             int tw, int th, GLuint tex, float alpha) {
    if (dw <= 0.0f || dh <= 0.0f || sw <= 0.0f || sh <= 0.0f) return;
    const float img_w = dw * (float)tw / sw, img_h = dh * (float)th / sh;
    luna_use_program(img_program);
    glUniform2f(img_loc.uResolution, LUNA_RRES_X, LUNA_RRES_Y);
    glUniform2f(img_loc.uPos, dx - g_render_off_x, dy - g_render_off_y);
    glUniform2f(img_loc.uSize, dw, dh);
    glUniform1f(img_loc.uRadius, -1.0f);
    glUniform1f(img_loc.uAlpha, alpha);
    /* FragPos.y is 0 at the bottom and the texture is loaded flipped. */
    glUniform2f(img_loc.uImgOrigin, -sx * dw / sw,
                -((float)th - sy - sh) * dh / sh);
    glUniform2f(img_loc.uImgDrawSize, img_w, img_h);
    glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
}

/* CSS border-image with `stretch`: nine pieces, the corners at their edge
 * widths, the edges stretched along one axis, the middle (with `fill`)
 * along both. */
static void draw_border_image(float x, float y, float w, float h, float scale,
                              LunaElement* e, float alpha) {
    if (!img_program || alpha <= 0.004f || w <= 0.0f || h <= 0.0f) return;
    if (!e->border_image_tex) {
        e->border_image_tex = load_or_get_texture(e->border_image_path);
        if (!e->border_image_tex) { e->has_border_image = 0; return; }
    }
    int tw = 0, th = 0;
    tex_cache_dims(e->border_image_tex, &tw, &th);
    if (tw <= 0 || th <= 0) return;
    float s[4];                                  /* t r b l, image px */
    for (int i = 0; i < 4; ++i) {
        const float basis = (i & 1) ? (float)tw : (float)th;
        s[i] = (e->bi_slice_pct >> i & 1) ? e->bi_slice[i] * basis / 100.0f
                                           : e->bi_slice[i];
        if (s[i] < 0.0f) s[i] = 0.0f;
    }
    float bw[4];
    for (int i = 0; i < 4; ++i)
        bw[i] = (e->bi_width[i] >= 0.0f ? e->bi_width[i] : s[i]) * scale;
    /* Edges that do not fit are scaled down together (CSS). */
    float f = 1.0f;
    if (bw[3] + bw[1] > w) f = fminf(f, w / (bw[3] + bw[1]));
    if (bw[0] + bw[2] > h) f = fminf(f, h / (bw[0] + bw[2]));
    for (int i = 0; i < 4; ++i) bw[i] *= f;
    const float sxs[3] = { 0.0f, s[3], (float)tw - s[1] };
    const float sws[3] = { s[3], (float)tw - s[3] - s[1], s[1] };
    const float sys[3] = { 0.0f, s[0], (float)th - s[2] };
    const float shs[3] = { s[0], (float)th - s[0] - s[2], s[2] };
    const float dxs[3] = { x, x + bw[3], x + w - bw[1] };
    const float dws[3] = { bw[3], w - bw[3] - bw[1], bw[1] };
    const float dys[3] = { y, y + bw[0], y + h - bw[2] };
    const float dhs[3] = { bw[0], h - bw[0] - bw[2], bw[2] };
    if (glActiveTexture_) glActiveTexture_(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, e->border_image_tex);
    luna_use_program(img_program);
    glUniform1i_(img_loc.uImage, 0);
    luna_bind_vao(g_rect_vao);
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) {
            if (r == 1 && c == 1 && !e->bi_fill) continue;
            draw_image_piece(dxs[c], dys[r], dws[c], dhs[r],
                             sxs[c], sys[r], sws[c], shs[r], tw, th,
                             e->border_image_tex, alpha);
        }
    glBindTexture(GL_TEXTURE_2D, 0);
}
...
static void draw_bg_layer(float x, float y, float w, float h,
    ...
    uni1f(bg_loc.uRotate, &bg_uni.uRotate, rotate);
...
/* Rotation for a draw_rect() without an element (radians, about the rect's
 * own centre); set only around a single call. */
static float g_draw_rotate = 0.0f;
...
void draw_rect_full(float x, float y, float w, float h,
    ...
    uni2f(bg_loc.uPos,  &bg_uni.uPos, x - g_render_off_x, y - g_render_off_y);
    uni2f(bg_loc.uSize, &bg_uni.uSize, w, h);
    uni1f(bg_loc.uRotate, &bg_uni.uRotate, ge ? ge->transform_rotate : g_draw_rotate);
...
/* One side of a per-side border.  The shader turns a rect about its own
 * centre, so for an element turned by rot the strip's centre is moved about
 * the element's centre first; the strip then turns with the same angle. */
static void draw_border_side(float ex, float ey, float ew, float eh,
                             float x, float y, float w, float h, float rot,
                             float r, float g, float b, float a) {
    if (rot != 0.0f) {
        float c = cosf(rot), sn = sinf(rot);
        float dx = x + w * 0.5f - (ex + ew * 0.5f), dy = y + h * 0.5f - (ey + eh * 0.5f);
        x = ex + ew * 0.5f + c * dx - sn * dy - w * 0.5f;
        y = ey + eh * 0.5f + sn * dx + c * dy - h * 0.5f;
        g_draw_rotate = rot;
    }
    draw_rect(x, y, w, h, r, g, b, a, 0, 0, 0, 0, 0, 0);
    g_draw_rotate = 0.0f;
}
...
static void flush_text_batch(GLuint tex, float* batch_buf, int* batch_count) {
    ...
    /* LUNA_UI_TRACE_TEXT: what the text pass actually submitted.  Text is the
     * one pass that can fail while every box still paints, so "were quads
     * issued, and against which atlas" is the first question to settle. */
    if (getenv("LUNA_UI_TRACE_TEXT")) {
        static int n;
        if (n++ < 8)
            fprintf(stderr, "[luna-ui] text batch: %d glyphs tex=%u "
                            "quad=(%.1f,%.1f)-(%.1f,%.1f) uv=(%.3f,%.3f)-(%.3f,%.3f) "
                            "res=%.0fx%.0f fb=%dx%d activeTexture=%s\n",
                    *batch_count, (unsigned)tex,
                    batch_buf[0], batch_buf[1], batch_buf[8], batch_buf[9],
                    batch_buf[2], batch_buf[3], batch_buf[10], batch_buf[11],
                    (double)LUNA_RRES_X, (double)LUNA_RRES_Y,
                    g_luna_fbw, g_luna_fbh,
                    glActiveTexture_ ? "resolved" : "NULL");
    }
    glBindTexture(GL_TEXTURE_2D, tex);
    glBindBuffer(GL_ARRAY_BUFFER, text_vbo);
...
void render_text_fx(const char* text, float x, float y, float box_w, float box_h, int align,
    ...
    float line_h = css_line_height > 0.0f ? css_line_height : css_normal_line_height(fsize);
    if (line_h < cap_h + 1.0f) line_h = cap_h + 1.0f;

    int max_lines = (int)floorf(box_h / line_h);
    if (max_lines < 1) max_lines = (box_h >= cap_h) ? 1 : 0;
    if (fx && fx->line_clamp > 0 && max_lines > fx->line_clamp)
        max_lines = fx->line_clamp;
    if (max_lines <= 0) {
        g_text_letter_spacing = 0.0f;
        text_metrics_end();
        return;
    ...
    int line_count = count_text_lines(atlas, text, box_w, line_h, max_lines,
                                      white_space,
                                      (text_overflow || (fx && fx->line_clamp > 0)),
                                      overflow_wrap);
    ...
    while (*p && line_no < max_lines) {
        ...
        if ((white_space == 1 || is_last_visible_line) &&
            (text_overflow == 1 || (fx && fx->line_clamp > 0)) && has_more) {
            float ell_w = measure_text_width(atlas, "…");
...
void cursor_position_callback(void* window, double xpos, double ypos) {
        ...
        if (g_scroll_drag_axis == 0) {
            ...
            if (vis) {
                ...
                if (scroll_range > 0.0f && max_scroll > 0.0f) {
                    ...
                    if (fabsf(sc->scroll_top - val) > 0.01f ||
                        fabsf(sc->scroll_dest_top - val) > 0.01f) {
                        sc->scroll_top = val;
                        sc->scroll_dest_top = val;
                        g_layout_dirty = 1;
                    }
            ...
            if (vis) {
                ...
                if (scroll_range > 0.0f && max_scroll > 0.0f) {
                    ...
                    if (ratio > 1.0f) ratio = 1.0f;
                    float val = ratio * max_scroll;
                    if (fabsf(sc->scroll_left - val) > 0.01f ||
                        fabsf(sc->scroll_dest_left - val) > 0.01f) {
                        sc->scroll_left = val;
                        sc->scroll_dest_left = val;
                        g_layout_dirty = 1;
                    }
    ...
    if (drag_target_idx != -1) {
        ...
        float old_rel_x = d->rel_x;
        float old_rel_y = d->rel_y;
        if (g_drag_mode == 2) {
            int p = d->parent_idx;
        ...
        /* High-rate mice may deliver duplicate/sub-pixel motion samples.  A
         * full document layout is required only when the stored drag origin
         * actually changed, not once for every raw pointer packet. */
        if (fabsf(d->rel_x - old_rel_x) > 0.001f ||
            fabsf(d->rel_y - old_rel_y) > 0.001f)
            g_layout_dirty = 1;
        return;
    }
...
static void LUNA_UNUSED framebuffer_size_callback(void* window, int width, int height) {
    (void)window;
    (void)width;
    (void)height;
    g_layout_dirty = 1; g_render_order_dirty = 1;
...
static void platform_sync_text_input(int idx) {
    if (!g_luna_platform.text_input) return;
    if (idx >= 0 && idx < elem_count && elements[idx].is_input) {
        LunaElement* e = &elements[idx];
        float x = e->x, y = e->y, w = e->w, h = e->h;
        if (w < 1.0f) w = 1.0f;
        if (h < 1.0f) h = e->font_size > 0.0f ? e->font_size * 1.5f : 24.0f;
...
static void focus_element_ex(int idx, int via_keyboard, int scroll_view) {
    ...
    restyle_descendants_of(old);
    restyle_descendants_of(idx);
    if ((old >= 0 && old < elem_count && elements[old].is_input) ||
        (idx >= 0 && idx < elem_count && elements[idx].is_input))
        platform_sync_text_input(idx);
}
...
static int element_is_focusable(int idx) {
    ...
    if (luna_strcasecmp(e->type, "button") == 0) return 1;
    if (luna_strcasecmp(e->type, "input") == 0 || luna_strcasecmp(e->type, "textarea") == 0) return 1;
...
static void key_callback(void* window, int key, int scancode, int action, int mods) {
     ...
     * This compact editor has no selection model yet, so copy/cut operate on
     * the complete value and paste inserts at the caret. */
    if (focused_is_input() && (action == LUNA_PRESS || action == LUNA_REPEAT)) {
        LunaElement* fe = &elements[g_focused_element_idx];
        int primary_mod = (mods & (LUNA_MOD_CONTROL | LUNA_MOD_SUPER)) != 0;
        if (primary_mod && action == LUNA_PRESS && key == 'C') {
            if (g_luna_platform.set_clipboard && !fe->input_password)
        ...
        if (primary_mod && action == LUNA_PRESS && key == 'V') {
            if (g_luna_platform.get_clipboard) {
                char* clip = g_luna_platform.get_clipboard();
                if (clip) {
                    input_insert_utf8(fe, clip, (int)strlen(clip));
                    free(clip);
                }
            }
            return;
...
static GLuint compile_shader(const char* src, GLenum type, const char* label) {
    ...
    if (!ok) {
        GLchar log[4096]; GLsizei n = 0;
        log[0] = '\0';
        glGetShaderInfoLog_(shader, (GLsizei)sizeof(log) - 1, &n, log);
        if (n < 0) n = 0;
        if ((size_t)n >= sizeof(log)) n = (GLsizei)sizeof(log) - 1;
        log[n] = '\0';
        fprintf(stderr, "[luna-ui] %s shader compile failed: %s\n",
                label ? label : "GLSL", log);
...
static GLuint create_shader_program(const char* vertex_src, const char* fragment_src,
                                    const char* label) {
    GLuint vs = compile_shader(vertex_src, GL_VERTEX_SHADER, label);
    GLuint fs; GLuint program; GLint ok = 0;
    if (!vs) return 0;
    fs = compile_shader(fragment_src, GL_FRAGMENT_SHADER, label);
    if (!fs) { glDeleteShader_(vs); return 0; }
    program = glCreateProgram();
    if (!program) { glDeleteShader_(vs); glDeleteShader_(fs); return 0; }
    ...
    if (!ok) {
        GLchar log[4096]; GLsizei n = 0;
        log[0] = '\0';
        glGetProgramInfoLog_(program, (GLsizei)sizeof(log) - 1, &n, log);
        if (n < 0) n = 0;
        if ((size_t)n >= sizeof(log)) n = (GLsizei)sizeof(log) - 1;
        log[n] = '\0';
        fprintf(stderr, "[luna-ui] %s program link failed: %s\n",
                label ? label : "GLSL", log);
...
#define LOAD(T, name) name = (T)g_luna_platform.get_proc(#name)
    ...
    LOAD(LunaPFNGLBUFFERDATAPROC,         glBufferData);
    LOAD(LunaPFNGLBUFFERSUBDATAPROC,      glBufferSubData);
    LOAD(LunaPFNGLVERTEXATTRIBPOINTERPROC,    glVertexAttribPointer);
    LOAD(LunaPFNGLENABLEVERTEXATTRIBARRAYPROC, glEnableVertexAttribArray);
    LOAD(LunaPFNGLDELETEBUFFERSPROC,      glDeleteBuffers);
    LOAD(LunaPFNGLBINDVERTEXARRAYPROC,    glBindVertexArray);
    LOAD(LunaPFNGLCREATESHADERPROC,       glCreateShader);
    LOAD(LunaPFNGLSHADERSOURCEPROC,       glShaderSource);
    LOAD(LunaPFNGLCOMPILESHADERPROC,      glCompileShader);
    glGetShaderiv_ = (LunaPFNGLGETSHADERIVPROC)g_luna_platform.get_proc("glGetShaderiv");
    glGetShaderInfoLog_ = (LunaPFNGLGETSHADERINFOLOGPROC)g_luna_platform.get_proc("glGetShaderInfoLog");
    glDeleteShader_ = (LunaPFNGLDELETESHADERPROC)g_luna_platform.get_proc("glDeleteShader");
    LOAD(LunaPFNGLCREATEPROGRAMPROC,      glCreateProgram);
    LOAD(LunaPFNGLATTACHSHADERPROC,       glAttachShader);
    LOAD(LunaPFNGLLINKPROGRAMPROC,        glLinkProgram);
    glGetProgramiv_ = (LunaPFNGLGETPROGRAMIVPROC)g_luna_platform.get_proc("glGetProgramiv");
    glGetProgramInfoLog_ = (LunaPFNGLGETPROGRAMINFOLOGPROC)g_luna_platform.get_proc("glGetProgramInfoLog");
    LOAD(LunaPFNGLUSEPROGRAMPROC,         glUseProgram);
    glDeleteProgram_ = (LunaPFNGLDELETEPROGRAMPROC)g_luna_platform.get_proc("glDeleteProgram");
    LOAD(LunaPFNGLGETUNIFORMLOCATIONPROC, glGetUniformLocation);
    LOAD(LunaPFNGLUNIFORM4FPROC,          glUniform4f);
    LOAD(LunaPFNGLUNIFORM2FPROC,          glUniform2f);
    LOAD(LunaPFNGLUNIFORM1FPROC,          glUniform1f);
    glUniform1i_ = (LunaPFNGLUNIFORM1IPROC)g_luna_platform.get_proc("glUniform1i");
    glActiveTexture_ = (LunaPFNGLACTIVETEXTUREPROC)g_luna_platform.get_proc("glActiveTexture");
    glGenFramebuffers_       = (LunaPFNGLGENFRAMEBUFFERSPROC)g_luna_platform.get_proc("glGenFramebuffers");
    glBindFramebuffer_       = (LunaPFNGLBINDFRAMEBUFFERPROC)g_luna_platform.get_proc("glBindFramebuffer");
    glFramebufferTexture2D_  = (LunaPFNGLFRAMEBUFFERTEXTURE2DPROC)g_luna_platform.get_proc("glFramebufferTexture2D");
    glDeleteFramebuffers_    = (LunaPFNGLDELETEFRAMEBUFFERSPROC)g_luna_platform.get_proc("glDeleteFramebuffers");
    glCheckFramebufferStatus_ = (LunaPFNGLCHECKFRAMEBUFFERSTATUSPROC)g_luna_platform.get_proc("glCheckFramebufferStatus");
...
#undef LOAD
    ...
    /* Name every entry point that did not resolve.  Most call sites do not
     * null-check, and the ones that do skip silently, so an unresolved pointer
     * shows up as a feature that quietly does nothing rather than as an error:
     * a host whose get_proc() misses glBufferSubData renders every box in the
     * right place and no text at all, because the glyph vertices are never
     * uploaded and the draw reads a buffer full of zeroes. */
    {
        const struct { const char* name; const void* fn; } probes[] = {
            { "glGenVertexArrays", (const void*)glGenVertexArrays },
            { "glBindVertexArray", (const void*)glBindVertexArray },
            ...
            { "glVertexAttribPointer", (const void*)glVertexAttribPointer },
            { "glEnableVertexAttribArray", (const void*)glEnableVertexAttribArray },
            { "glDrawArrays", (const void*)glDrawArrays },
            { "glUniform2f", (const void*)glUniform2f },
            { "glUniform4f", (const void*)glUniform4f },
            { "glUniform1i", (const void*)glUniform1i_ },
            { "glActiveTexture", (const void*)glActiveTexture_ },
        ...
        for (size_t i = 0; i < sizeof(probes) / sizeof(probes[0]); ++i)
            if (!probes[i].fn)
                fprintf(stderr, "[luna-ui] GL entry point unresolved: %s\n",
                        probes[i].name);
    }
...
void luna_clipboard_set(const char* utf8) {
    if (g_luna_platform.set_clipboard) g_luna_platform.set_clipboard(utf8 ? utf8 : "");
}
char* luna_clipboard_get(void) {
    return g_luna_platform.get_clipboard ? g_luna_platform.get_clipboard() : NULL;
}
void luna_clipboard_free(char* utf8) { free(utf8); }
float luna_platform_scale(void) {
    float s = g_luna_platform.get_scale ? g_luna_platform.get_scale() : 1.0f;
...
void luna_reset_document(void) {
    ...
    g_dom_uid_seq = 0;
    g_dom_uid_dirty = 1;
    /* Keyed by element index: a scrollbar slot or an "already active" flag
       left from the previous document names an unrelated element of the next
       one (the slot then repositions it as a scrollbar). */
    memset(g_sb_slots, 0, sizeof(g_sb_slots));
    memset(g_visual_active_flag, 0, sizeof(g_visual_active_flag));
...
static int* g_dom_uid_map = NULL;       /* uid -> index + 1 */
static unsigned g_dom_uid_map_cap = 0;

static int dom_valid(int idx) { return idx >= 0 && idx < elem_count; }

static void dom_uid_map_sync(void) {
    if (!g_dom_uid_dirty) return;
    unsigned need = g_dom_uid_seq + 1u;
    if (need > g_dom_uid_map_cap) {
        unsigned cap = g_dom_uid_map_cap ? g_dom_uid_map_cap : 256u;
        while (cap < need) cap *= 2u;
        int* grown = (int*)realloc(g_dom_uid_map, sizeof(int) * cap);
        if (!grown) return;
        g_dom_uid_map = grown;
        g_dom_uid_map_cap = cap;
    }
    memset(g_dom_uid_map, 0, sizeof(int) * need);
    for (int i = 0; i < elem_count; i++) {
        unsigned uid = elements[i].dom_uid;
        if (uid && uid < need) g_dom_uid_map[uid] = i + 1;
    }
    g_dom_uid_dirty = 0;
}

unsigned luna_dom_uid(int idx) { return dom_valid(idx) ? elements[idx].dom_uid : 0u; }

void luna_set_web_compat(int enabled) { g_luna_web_compat = enabled ? 1 : 0; }

int luna_dom_create_text(const char* text) {
    if (elem_count >= MAX_ELEMENTS || !luna_ensure_element_capacity(elem_count + 1)) return -1;
    int idx = elem_count;
    LunaElement* t = &elements[idx];
    memset(t, 0, sizeof(*t));
    t->id_idx = idx;
    t->parent_idx = -1;
    t->dom_detached = 1;
...
int luna_dom_find(unsigned uid) {
    if (!uid || uid > g_dom_uid_seq) return -1;
    dom_uid_map_sync();
    if (g_dom_uid_dirty || uid >= g_dom_uid_map_cap) return -1;
    return g_dom_uid_map[uid] - 1;
}

static int dom_is_ancestor(int anc, int idx) {
    for (int p = elements[idx].parent_idx; p != -1; p = elements[p].parent_idx)
        if (p == anc) return 1;
    return 0;
}

/* Every cache that is keyed by, or holds, an element index. */
static void dom_invalidate_index_state(void) {
    g_scroll_tick_count = 0;
    g_scroll_container_count = 0;
    g_css_anim_count = 0;
    for (int i = 0; i < g_visual_active_count; i++) {
        int v = g_visual_active_idx[i];
        if (v >= 0 && v < MAX_ELEMENTS) g_visual_active_flag[v] = 0;
    }
    g_visual_active_count = 0;
    g_visual_scan_needed = 1;
    g_activity_registry_dirty = 1;
...
/* Reorders elements[] so that new position k holds old element order[k], and
 * renumbers everything that refers to an element by index. */
static int dom_apply_order(const int* order) {
    int n = elem_count;
    int* remap = (int*)malloc(sizeof(int) * (size_t)n);
    LunaElement* moved = (LunaElement*)malloc(sizeof(LunaElement) * (size_t)n);
    int (*slots)[4] = (int (*)[4])calloc((size_t)n, sizeof(int[4]));
    if (!remap || !moved || !slots) { free(remap); free(moved); free(slots); return 0; }
    for (int k = 0; k < n; k++) { remap[order[k]] = k; moved[k] = elements[order[k]]; }
    memcpy(elements, moved, sizeof(LunaElement) * (size_t)n);
    free(moved);
#define DOM_RM(v) (((v) >= 0 && (v) < n) ? remap[(v)] : (v))
    for (int k = 0; k < n; k++) {
        LunaElement* e = &elements[k];
        e->parent_idx = DOM_RM(e->parent_idx);
        e->id_idx = k;
        if (e->luna_internal) e->sb_host_idx = DOM_RM(e->sb_host_idx);
    }
    for (int i = 0; i < n && i < MAX_ELEMENTS; i++)
        for (int a = 0; a < 4; a++)
            if (g_sb_slots[i][a]) slots[remap[i]][a] = DOM_RM(g_sb_slots[i][a]);
    memset(g_sb_slots, 0, sizeof(g_sb_slots));
    memcpy(g_sb_slots, slots, sizeof(int[4]) * (size_t)n);
    free(slots);
    g_focused_idx = DOM_RM(g_focused_idx);
    g_focused_element_idx = DOM_RM(g_focused_element_idx);
    g_focus_before_trap = DOM_RM(g_focus_before_trap);
    for (int i = 0; i < g_focus_trap_count; i++)
        g_focus_traps[i].idx = DOM_RM(g_focus_traps[i].idx);
    drag_target_idx = DOM_RM(drag_target_idx);
    g_scroll_drag_idx = DOM_RM(g_scroll_drag_idx);
    g_scroll_hover_idx = DOM_RM(g_scroll_hover_idx);
    for (int i = 0; i < g_hover_chain_count; i++)
        g_hover_chain[i] = DOM_RM(g_hover_chain[i]);
#undef DOM_RM
    if (++g_hover_epoch == 0) g_hover_epoch = 1;
    free(remap);
    dom_invalidate_index_state();
    return 1;
}

/* Restyles idx and its subtree (selectors see ancestors and siblings). */
static void dom_restyle_subtree(int idx) {
    if (!dom_valid(idx)) return;
    if (g_has_structural_selectors) {
        for (int i = 0; i < elem_count; i++)
            if (!elements[i].luna_internal) (void)restyle_element_checked(&elements[i]);
    } else {
        (void)restyle_element_checked(&elements[idx]);
        for (int i = 0; i < elem_count; i++)
            if (!elements[i].luna_internal && dom_is_ancestor(idx, i))
                (void)restyle_element_checked(&elements[i]);
    }
    generate_pseudo_elements();
    memset(g_intrinsic_width_valid, 0, sizeof(g_intrinsic_width_valid));
...
void luna_dom_restyle(int idx) { dom_restyle_subtree(idx); }

int luna_dom_create(const char* tag, const char* attrs) {
    if (!tag || !tag[0] || strlen(tag) >= sizeof(((LunaElement*)0)->type)) return -1;
    char* html = (char*)malloc(strlen(tag) + (attrs ? strlen(attrs) : 0) + 8);
    if (!html) return -1;
    sprintf(html, "<%s%s%s>", tag, attrs && attrs[0] && !isspace((unsigned char)attrs[0]) ? " " : "",
            attrs ? attrs : "");
    int before = elem_count;
    /* Document-level structure tags (html, head, body, script, style, ...) are
       not rendered elements; a caller keeps those itself. */
    if (luna_strcasecmp(tag, "body") == 0 || is_ignored_element(tag) ||
        is_semantic_shell_element(tag)) { free(html); return -1; }
    parse_html_into(html, -1);
    free(html);
    if (elem_count == before) return -1;
    int idx = before;
    elements[idx].parent_idx = -1;
    elements[idx].dom_detached = 1;
    g_dom_uid_dirty = 1;
    g_id_map_ready = 0;
    g_probe_prepared = 0;
    return idx;
...
int luna_dom_is_connected(int idx) {
    if (!dom_valid(idx)) return 0;
    for (int p = idx; p != -1; p = elements[p].parent_idx)
        if (elements[p].dom_detached) return 0;
    return 1;
}
...
int luna_dom_children(int idx, int* out, int cap) {
    ...
    for (int i = 0; i < elem_count; i++) {
        const LunaElement* e = &elements[i];
        if (e->parent_idx != idx || e->luna_internal || e->generated_pseudo) continue;
        if (idx == -1 && e->dom_detached) continue;
...
int luna_dom_insert(int parent, int child, int before) {
    if (!dom_valid(child) || (parent != -1 && !dom_valid(parent))) return -1;
    if (parent == child || (parent != -1 && dom_is_ancestor(child, parent))) return -1;
    if (before != -1 && (!dom_valid(before) || elements[before].parent_idx != parent ||
                         before == child)) return -1;
    int n = elem_count;
    /* The moving set: child and everything below it, in their current
       (document) order. */
    unsigned char* in_set = (unsigned char*)calloc((size_t)n, 1);
    int* order = (int*)malloc(sizeof(int) * (size_t)n);
    if (!in_set || !order) { free(in_set); free(order); return -1; }
    in_set[child] = 1;
    for (int i = 0; i < n; i++) if (dom_is_ancestor(child, i)) in_set[i] = 1;
    /* Destination: before `before`, or after the last element of parent's
       subtree (the document's end for a root). */
    int dest = n;
    if (before != -1) dest = before;
    else if (parent != -1) {
        dest = parent + 1;
        for (int i = parent + 1; i < n; i++)
            if (!in_set[i] && dom_is_ancestor(parent, i)) dest = i + 1;
    }
    int k = 0;
    for (int i = 0; i <= n; i++) {
        if (i == dest)
            for (int j = 0; j < n; j++) if (in_set[j]) order[k++] = j;
        if (i < n && !in_set[i]) order[k++] = i;
    }
    int new_child = -1;
    elements[child].parent_idx = parent;
    elements[child].dom_detached = 0;
    int ok = dom_apply_order(order);
    if (ok) for (int i = 0; i < n; i++) if (order[i] == child) { new_child = i; break; }
    free(in_set);
    free(order);
    if (new_child < 0) return -1;
    dom_restyle_subtree(new_child);
    return new_child;
}

int luna_dom_remove(int idx) {
    if (!dom_valid(idx) || elements[idx].luna_internal) return -1;
    if (g_focused_element_idx == idx || (g_focused_element_idx >= 0 &&
                                          dom_is_ancestor(idx, g_focused_element_idx)))
        g_focused_element_idx = -1;
    elements[idx].parent_idx = -1;
    elements[idx].dom_detached = 1;
    g_hover_chain_count = 0;
...
int luna_dom_set_inner_html(int idx, const char* html) {
    if (!dom_valid(idx)) return -1;
    int kids[256];
    int nk;
    while ((nk = luna_dom_children(idx, kids, 256)) > 0)
        for (int i = 0; i < nk && i < 256; i++) luna_dom_remove(kids[i]);
    elements[idx].text[0] = '\0';
    elements[idx].direct_text_before_children = 0;
    if (!html || !html[0]) { g_layout_dirty = 1; g_probe_prepared = 0; return 0; }
    unsigned uid = elements[idx].dom_uid;
    int before = elem_count;
    /* Text that precedes the first child tag belongs to idx itself. */
    const char* lt = strchr(html, '<');
    size_t lead = lt ? (size_t)(lt - html) : strlen(html);
    if (lead) append_direct_text(idx, html, html + lead);
    if (elements[idx].text[0] == ' ')
        memmove(elements[idx].text, elements[idx].text + 1, strlen(elements[idx].text));
    elements[idx].direct_text_before_children = elements[idx].text[0] != '\0';
    parse_html_into(html + lead, idx);
    int after = elem_count;
    /* Top-level uids, in order, to re-find them after each move. */
    int count = 0;
    unsigned* tops = (unsigned*)malloc(sizeof(unsigned) * (size_t)(after - before + 1));
    if (!tops) return -1;
    for (int i = before; i < after; i++)
        if (elements[i].parent_idx == idx && elements[i].dom_uid) tops[count++] = elements[i].dom_uid;
    for (int i = 0; i < count; i++) {
        int p = luna_dom_find(uid), c = luna_dom_find(tops[i]);
        if (p >= 0 && c >= 0) luna_dom_insert(p, c, -1);
    }
    free(tops);
    return count;
}

/* Attribute list editing.  dom_attrs holds ` name="value"` pairs. */
static const char* dom_attr_next(const char* s, char* name, int ncap,
                                 const char** vbeg, const char** vend) {
    while (*s && isspace((unsigned char)*s)) s++;
    if (!*s) return NULL;
    int n = 0;
    while (*s && !isspace((unsigned char)*s) && *s != '=' && *s != '>') {
        if (n < ncap - 1) name[n++] = (char)tolower((unsigned char)*s);
        s++;
    }
    name[n] = '\0';
    *vbeg = *vend = NULL;
...
const char* luna_dom_attrs(int idx) {
    return dom_valid(idx) && elements[idx].dom_attrs ? elements[idx].dom_attrs : "";
}

int luna_dom_get_attr(int idx, const char* name, char* out, int cap) {
    if (!dom_valid(idx) || !name) return 0;
    const char* s = elements[idx].dom_attrs;
    char key[64];
    const char *vb, *ve;
    while (s && (s = dom_attr_next(s, key, sizeof key, &vb, &ve)) != NULL) {
        if (!key[0]) { if (*s) s++; continue; }
        if (luna_strcasecmp(key, name) != 0) continue;
        if (out && cap > 0) {
            int n = vb ? (int)(ve - vb) : 0;
            if (n >= cap) n = cap - 1;
            if (n > 0) memcpy(out, vb, (size_t)n);
            out[n] = '\0';
...
/* Rebuilds dom_attrs without `name`, then appends it when value != NULL. */
static int dom_attr_write(int idx, const char* name, const char* value) {
    const char* s = elements[idx].dom_attrs;
    size_t need = (s ? strlen(s) : 0) + strlen(name) + (value ? strlen(value) * 6 : 0) + 8;
    ...
    while (s && *s) {
        const char* start = s;
        s = dom_attr_next(s, key, sizeof key, &vb, &ve);
        if (!s) break;
        if (!key[0]) { if (*s) s++; continue; }
        if (luna_strcasecmp(key, name) == 0) continue;
        while (start < s && isspace((unsigned char)*start)) start++;
        out[w++] = ' ';
        memcpy(out + w, start, (size_t)(s - start));
        w += (size_t)(s - start);
    }
    if (value) {
        w += (size_t)sprintf(out + w, " %s=\"", name);
        for (const char* v = value; *v; v++) {
            const char* esc = *v == '"' ? "&quot;" : *v == '&' ? "&amp;" : NULL;
            if (esc) { memcpy(out + w, esc, strlen(esc)); w += strlen(esc); }
            else out[w++] = *v;
        }
        out[w++] = '"';
    }
    out[w] = '\0';
    free(elements[idx].dom_attrs);
    elements[idx].dom_attrs = out;
    return 0;
}

/* The attributes the renderer reads at parse time, applied to a live node. */
static void dom_attr_apply(int idx, const char* name, const char* value) {
    LunaElement* e = &elements[idx];
    const char* v = value ? value : "";
    if (!luna_strcasecmp(name, "id")) {
        snprintf(e->id, sizeof(e->id), "%s", v);
        g_id_map_ready = 0; g_id_map_built = 0;
    } else if (!luna_strcasecmp(name, "class")) {
        snprintf(e->class_name, sizeof(e->class_name), "%s", v);
    } else if (!luna_strcasecmp(name, "style")) {
        snprintf(e->inline_style, sizeof(e->inline_style), "%s", v);
        e->has_inline_style = v[0] != '\0';
    } else if (!luna_strcasecmp(name, "src") && !luna_strcasecmp(e->type, "img")) {
        e->has_bg_image = v[0] != '\0';
        snprintf(e->bg_image_path, sizeof(e->bg_image_path), "%s", v);
        e->bg_image_tex = 0;
    } else if (!luna_strcasecmp(name, "placeholder")) {
        snprintf(e->placeholder, sizeof(e->placeholder), "%s", v);
    } else if (!luna_strcasecmp(name, "value") && e->is_input) {
        set_text(idx, v);
        e->caret = (int)strlen(e->text);
    } else if (!luna_strcasecmp(name, "type") && !luna_strcasecmp(e->type, "input")) {
        e->input_password = !luna_strcasecmp(v, "password");
    } else if (!luna_strcasecmp(name, "tabindex")) {
        e->tabindex = value ? atoi(v) : -2;
    } else if (!luna_strcasecmp(name, "aria-label")) {
        snprintf(e->aria_label, sizeof(e->aria_label), "%s", v);
    } else if (!luna_strcasecmp(name, "role")) {
        snprintf(e->role, sizeof(e->role), "%s", v);
    } else if (!luna_strcasecmp(name, "data-tab")) {
        snprintf(e->data_tab, sizeof(e->data_tab), "%s", v);
    } else if (!luna_strcasecmp(name, "aria-hidden")) {
        e->aria_hidden = value && (strcmp(v, "true") == 0 || strcmp(v, "1") == 0 || !v[0]);
    } else if (!luna_strcasecmp(name, "inert")) {
        e->inert = value != NULL;
    }
}

int luna_dom_set_attr(int idx, const char* name, const char* value) {
    if (!dom_valid(idx) || !name || !name[0] || strlen(name) >= 64) return -1;
    if (!value) value = "";
    if (dom_attr_write(idx, name, value) != 0) return -1;
    dom_attr_apply(idx, name, value);
    dom_restyle_subtree(idx);
    return 0;
}

int luna_dom_remove_attr(int idx, const char* name) {
    if (!dom_valid(idx) || !name || !name[0] || strlen(name) >= 64) return -1;
    if (!luna_dom_get_attr(idx, name, NULL, 0)) return 0;
    if (dom_attr_write(idx, name, NULL) != 0) return -1;
    dom_attr_apply(idx, name, NULL);
    dom_restyle_subtree(idx);
    return 0;
}
...
int luna_needs_redraw(double now, double dt) {
    ...
    /* Event-driven hosts may sleep after the current swap.  Queue exactly one
       wake-up while visual interpolation/keyframes are live; static pages do
       not pay for a timer or a continuous render loop. */
    if ((mask & LUNA_REDRAW_ANIM) && g_luna_platform.request_redraw)
        g_luna_platform.request_redraw();
    return mask;
}
...
static void apply_backdrop_blur(float ex, float ey, float ew, float eh,
    ...
    /* CSS colours arrive in display-referred sRGB bytes/floats and Luna's
       shaders intentionally blend those values directly, matching the fast
       browser 2D path.  A host can leave GL_FRAMEBUFFER_SRGB enabled; that
       would encode the already encoded CSS values a second time and wash out
       saturated colours (#2457d6 became roughly #6aa0ed). */
...
void luna_render(int fbw, int fbh) {
    ...
    for (int ri = 0; ri < elem_count; ri++) {
        ...
        if (e->bg_layer_count > 1 && e->bg_layers) {
            for (int li = e->bg_layer_count - 1; li >= 0; li--)
                draw_bg_layer(dx, dy, dw, dh, rad4, eff_op,
                              e->transform_rotate, &e->bg_layers[li]);
        ...
        {
            const float rot = e->transform_rotate;
            if (e->has_border_top && e->border_top_w > 0.0f && e->border_top_a * eff_op > 0.004f)
                draw_border_side(dx, dy, dw, dh, dx, dy, dw, e->border_top_w * scale, rot,
            ...
            if (e->has_border_bottom && e->border_bottom_w > 0.0f && e->border_bottom_a * eff_op > 0.004f)
                draw_border_side(dx, dy, dw, dh, dx, dy + dh - e->border_bottom_w * scale, dw,
                                 e->border_bottom_w * scale, rot,
            ...
            if (e->has_border_left && e->border_left_w > 0.0f && e->border_left_a * eff_op > 0.004f)
                draw_border_side(dx, dy, dw, dh, dx, dy, e->border_left_w * scale, dh, rot,
            ...
            if (e->has_border_right && e->border_right_w > 0.0f && e->border_right_a * eff_op > 0.004f)
                draw_border_side(dx, dy, dw, dh, dx + dw - e->border_right_w * scale, dy,
                                 e->border_right_w * scale, dh, rot,
                                 e->border_right_r, e->border_right_g, e->border_right_b, e->border_right_a * eff_op);
        }
        if (e->has_border_image && e->border_image_path[0])
            draw_border_image(dx, dy, dw, dh, scale, e, eff_op);
    ...
    /* Hand the host back the default vertex array.  Leaving ours bound let a
     * host's next glVertexAttribPointer/glDisableVertexAttribArray edit it —
     * a compositor drawing its own quad disabled attribute 0 of the rect VAO,
     * and every later pass drew nothing without raising a GL error. */
    luna_bind_vao(0);
}
...
int luna_mouse_move_changed(double x, double y) {
    ...
    int dragging = luna_pointer_dragging();
...
/* Setting a field's value from code leaves the caret after it, as assigning
 * HTMLInputElement.value does.  Leaving it where it was (0 in a freshly
 * parsed field) made every character typed next land in front of the text
 * already there: a host that re-seeds its field on each document rebuild saw
 * its input come out reversed. */
void luna_set_value(int idx, const char* value) {
    g_probe_prepared = 0;
    set_text(idx, value ? value : "");
    if (idx >= 0 && idx < elem_count && elements[idx].is_input) {
        elements[idx].caret = (int)strlen(elements[idx].text);
        input_ensure_caret(&elements[idx]);
    }
}
...
static void cache_uniform_locations(void) {
    ...
    bg_loc.uRotate      = glGetUniformLocation(bg_program, "uRotate");
...
int luna_init(const LunaInitConfig* cfg) {
    ...
    bg_program = create_shader_program(bg_vs, bg_fs, "background");
    text_program = create_shader_program(text_vs, text_fs, "text");
    shadow_program = create_shader_program(bg_vs, shadow_fs, "shadow");
    img_program = create_shader_program(bg_vs, img_fs, "image");
    blur_program = create_shader_program(bg_vs, blur_fs, "blur");
    backdrop_program = create_shader_program(bg_vs, backdrop_fs, "backdrop");
    if (!bg_program || !text_program || !shadow_program || !img_program ||
        !blur_program || !backdrop_program) {
        if (bg_program) glDeleteProgram_(bg_program);
        if (text_program) glDeleteProgram_(text_program);
        if (shadow_program) glDeleteProgram_(shadow_program);
        if (img_program) glDeleteProgram_(img_program);
        if (blur_program) glDeleteProgram_(blur_program);
        if (backdrop_program) glDeleteProgram_(backdrop_program);
        bg_program = text_program = shadow_program = img_program = 0;
        blur_program = backdrop_program = 0;
        return 0;
    }
...
void luna_shutdown(void) {
    ...
    g_tex_count = 0;
    for (int i = 0; i < NUM_FONT_SIZES; ++i) {
        if (font_regular[i].loaded && font_regular[i].tex)
            glDeleteTextures(1, &font_regular[i].tex);
        if (font_bold_atlas[i].loaded && font_bold_atlas[i].tex)
            glDeleteTextures(1, &font_bold_atlas[i].tex);
    }
    if (g_dyn_tex) glDeleteTextures(1, &g_dyn_tex);
    if (g_blur_tex[0]) glDeleteTextures(1, &g_blur_tex[0]);
    if (g_blur_tex[1]) glDeleteTextures(1, &g_blur_tex[1]);
    if (glDeleteFramebuffers_) {
        if (g_blur_fbo[0]) glDeleteFramebuffers_(1, &g_blur_fbo[0]);
        if (g_blur_fbo[1]) glDeleteFramebuffers_(1, &g_blur_fbo[1]);
    }
    if (g_rect_vbo) glDeleteBuffers(1, &g_rect_vbo);
    if (text_vbo) glDeleteBuffers(1, &text_vbo);
    if (g_rect_vao) glDeleteVertexArrays(1, &g_rect_vao);
    ...
    free(g_font_ttf); g_font_ttf = NULL;
    free(g_bold_font_ttf); g_bold_font_ttf = NULL;
    free(g_cjk_font_ttf); g_cjk_font_ttf = NULL;
    ...
    bg_program = text_program = shadow_program = img_program = 0;
    blur_program = backdrop_program = 0;
    g_rect_vao = g_rect_vbo = text_vao = text_vbo = 0;
    g_dyn_tex = 0; g_blur_tex[0] = g_blur_tex[1] = 0;
    g_blur_fbo[0] = g_blur_fbo[1] = 0;
    font_loaded = bold_font_loaded = 0;
    memset(&g_luna_platform, 0, sizeof(g_luna_platform));
}
...
#if !defined(LUNA_UI_NO_PLATFORM) && defined(LUNA_UI_IMPLEMENTATION)
#  define LUNA_UI_PLATFORM_BODY 1
#  if defined(__EMSCRIPTEN__)
#    include "luna_web.h"
#  elif defined(__ANDROID__)
#    include "luna_android.h"
