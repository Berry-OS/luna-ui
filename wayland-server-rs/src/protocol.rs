...
impl Interface {
    pub fn request_sig(&self, opcode: u16) -> Option<&'static str> {
        self.requests.get(opcode as usize).map(|m| m.signature)
    }
    pub fn request_name(&self, opcode: u16) -> &'static str {
        self.requests.get(opcode as usize).map(|m| m.name).unwrap_or("?")
    }
}
...
pub static WL_SURFACE: Interface = Interface {
    ...
    requests: &[
        ...
        m!("damage", "iiii"),
        m!("frame", "n"),
        m!("set_opaque_region", "?o"),
        m!("set_input_region", "?o"),
        m!("commit", ""),
        m!("set_buffer_transform", "i"),
        m!("set_buffer_scale", "i"),
        m!("damage_buffer", "iiii"),
        m!("offset", "ii"),
    ],
...
pub static WL_DATA_DEVICE_MANAGER: Interface = Interface {
    ...
    version: 3,
    requests: &[
        m!("create_data_source", "n"),
        m!("get_data_device", "no"),
    ],
    events: &[],
};
...
pub static XDG_WM_BASE: Interface = Interface {
    ...
    requests: &[
        m!("destroy", ""),
        m!("create_positioner", "n"),
        m!("get_xdg_surface", "no"),
        m!("pong", "u"),
    ],
    events: &[m!("ping", "u")],
};
...
pub static XDG_POSITIONER: Interface = Interface {
    ...
    requests: &[
        m!("destroy", ""),
        m!("set_size", "ii"),
        m!("set_anchor_rect", "iiii"),
        m!("set_anchor", "u"),
        m!("set_gravity", "u"),
        m!("set_constraint_adjustment", "u"),
        m!("set_offset", "ii"),
        m!("set_reactive", ""),
        m!("set_parent_size", "ii"),
        m!("set_parent_configure", "u"),
    ],
    events: &[],
};
