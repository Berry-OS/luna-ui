...
fn main() {
    ...
    while let Some(a) = args.next() {
        match a.as_str() {
            ...
            "-h" | "--help" => {
                eprintln!(
                    "vespera-server [--socket NAME] [--backend software|dri|webgl] \
                     [--screenshot PATH] [--fbdev DEV] [--size WxH] [--port PORT]"
                );
                return;
            }
            _ => eprintln!("[vespera-server] unknown argument: {}", a),
        }
    }

    let backend: Box<dyn Backend> = match backend_kind.as_str() {
        #[cfg(all(target_os = "linux", feature = "dri"))]
        "dri" => match wayland_server::render::dri::DriBackend::open_any() {
            Some(b) => {
                eprintln!("[vespera-server] using DRI (DRM/KMS) backend");
                Box::new(b)
            }
            None => {
                eprintln!("[vespera-server] DRI init failed, falling back to software");
        ...
        "dri" => {
            eprintln!(
                "[vespera-server] dri backend requires rebuild with feature=\"dri\"; \
                 using software"
            );
            Box::new(make_software(width, height, screenshot.clone(), fbdev.clone()))
        }
        #[cfg(feature = "webgl")]
        "webgl" => {
            eprintln!("[vespera-server] using WebGL browser streaming backend");
            Box::new(wayland_server::render::webgl_server::WebGlServerBackend::new(
                width, height, port,
            ))
        }
        ...
        "webgl" => {
            eprintln!(
                "[vespera-server] webgl backend requires rebuild with feature=\"webgl\" \
                 (cargo build --features webgl); using software"
            );
            Box::new(make_software(width, height, screenshot.clone(), fbdev.clone()))
        }
        _ => Box::new(make_software(width, height, screenshot.clone(), fbdev.clone())),
    };

    if let Err(e) = wayland_server::run(&socket, backend) {
        eprintln!("[vespera-server] failed to start: {}", e);
        std::process::exit(1);
    }
}
