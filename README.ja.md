...
| **luna-compositor** | `vespera-server` | DRM/KMS 上の Wayland コンポジタ（Xorg/Weston の代替） |
...
systemd / init
  └─ luna-session
       ...
       ├─ luna-shell       (luna-shell --desktop, Wayland クライアント)
...
│       ├── bin/server.rs            # vespera-server バイナリ (引数解釈)
...
make demo
# 内部動作:
#   vespera-server --socket wayland-1 をバックグラウンド起動
#   WAYLAND_DISPLAY=wayland-1 LD_PRELOAD=libwayland-client.so.0 で hello-gtk を接続
#   合成結果は /tmp/vespera.ppm に毎フレーム書き出される
```
...
./target/debug/vespera-server --backend dri  # KMS が使える Linux コンソール上で
```
